...
task("build:es5", () => configHelper.createBuild("es5", "ie > 9"));
...
task("write-polymer-config", (cb) =>
  writeFile(
    resolvePath(__dirname, "../build/polymer.json"),
    configHelper.output,
    cb
  )
);
...
task(
  "build",
  series(parallel("build:es5", "build:es6"), "write-polymer-config")
);
