...
export const MODELTYPES = {
  ...
  "https://w3id.org/okn/o/sdm#TheoryGuidedEmpiricalModel":
    "Theory-guided Empirical Model",
...
export const getModelTypeNames = (types: string[]): string[] => {
  return types
    .filter(
      (t: string) => t != "https://w3id.org/okn/o/sdm#Model" && t != "Model"
    )
    .map((t: string) =>
      MODELTYPES[t]
        ? MODELTYPES[t]
        : t.replace("https://w3id.org/okn/o/sdm#", "")
    );
};
...
export const getURL = (
  model: Model | string,
  ver?: SoftwareVersion | string,
  cfg?: ModelConfiguration | string,
  setup?: ModelConfigurationSetup | string
): string => {
...
export const isMainRegion = (region: Region): boolean => {
  return (
    region.id === "https://w3id.org/okn/i/mint/Kyrgyzstan" ||
    ...
    region.id ===
      "https://w3id.org/okn/i/mint/ad19b302-f69a-4325-8214-4e03e06850d0" || //California
    region.id === "https://w3id.org/okn/i/mint/Texas"
  );
};

export const setupInRegion = (
  setup: ModelConfigurationSetup,
  parentRegionId: string,
  allRegions?: IdMap<Region>
) => {
  if (!setup.hasRegion || setup.hasRegion.length == 0) return true;
  let regions = allRegions
    ? setup.hasRegion.map((region: Region) => allRegions[region.id])
    : setup.hasRegion;
...
export const sortSetups = (
  setup1: ModelConfigurationSetup,
  setup2: ModelConfigurationSetup
) => {
...
export const getLatestVersion = (
  ver1: SoftwareVersion,
  ver2: SoftwareVersion
): SoftwareVersion => {
  if (!ver1) return ver2;
  if (!ver2) return ver1;
...
export const getLatestSetup = (
  ...
  if (taggedAs(setup1, TAG_LATEST) || taggedAs(setup2, TAG_DEPRECATED))
    return setup1;
  if (taggedAs(setup2, TAG_LATEST) || taggedAs(setup1, TAG_DEPRECATED))
    return setup2;
