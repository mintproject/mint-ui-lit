...
import { CausalDiagramApi, ConfigurationSetupApi, DataTransformationApi, DataTransformationSetupApi,
         DatasetSpecificationApi, EmpiricalModelApi, EmulatorApi, EquationApi, FundingInformationApi, GeoCoordinatesApi,
         GeoShapeApi, GridApi, HybridModelApi, ImageApi, InterventionApi, ModelApi, ModelConfigurationApi,
         ModelConfigurationSetupApi, NumericalIndexApi, OrganizationApi, ParameterApi, PersonApi, PointBasedGridApi,
...
export class UserCatalog {
    ...
    private _dataTransformationSetupApi : DefaultReduxApi<DataTransformationSetup,DataTransformationSetupApi>;
    private _datasetSpecificationApi : DefaultReduxApi<DatasetSpecification,DatasetSpecificationApi>;
    private _empiricalModelApi : DefaultReduxApi<EmpiricalModel,EmpiricalModelApi>;
    private _emulatorApi : DefaultReduxApi<Emulator,EmulatorApi>;
    private _equationApi : DefaultReduxApi<Equation,EquationApi>;
    private _fundingInformationApi : DefaultReduxApi<FundingInformation,FundingInformationApi>;
    private _geoCoordinatesApi : DefaultReduxApi<GeoCoordinates,GeoCoordinatesApi>;
    ...
    private _softwareConfigurationApi : DefaultReduxApi<SoftwareConfiguration,SoftwareConfigurationApi>;
    private _softwareImageApi : DefaultReduxApi<SoftwareImage,SoftwareImageApi>;
    private _softwareVersionApi : DefaultReduxApi<SoftwareVersion,SoftwareVersionApi>;
    private _sourceCodeApi : DefaultReduxApi<SourceCode,SourceCodeApi>;
    private _spatialResolutionApi : DefaultReduxApi<SpatialResolution,SpatialResolutionApi>;
    private _spatiallyDistributedGridApi : DefaultReduxApi<SpatiallyDistributedGrid, SpatiallyDistributedGridApi>;
    private _standardVariableApi : DefaultReduxApi<StandardVariable,StandardVariableApi>;
    private _theoryGuidedModelApi : DefaultReduxApi<TheoryGuidedModel,TheoryGuidedModelApi>;
    ...
    private _variablePresentationApi : DefaultReduxApi<VariablePresentation,VariablePresentationApi>;
    ...
    public get dataTransformation () : DefaultReduxApi<DataTransformation,DataTransformationApi> {
        if (!this._dataTransformationApi)
            this._dataTransformationApi = new DefaultReduxApi<DataTransformation,DataTransformationApi> (
                DataTransformationApi, this.username, this.configuration
            );
        return this._dataTransformationApi;
    };

    public get dataTransformationSetup () : DefaultReduxApi<DataTransformationSetup,DataTransformationSetupApi> {
        if (!this._dataTransformationSetupApi) 
            this._dataTransformationSetupApi = new DefaultReduxApi<DataTransformationSetup,DataTransformationSetupApi> (
                DataTransformationSetupApi, this.username, this.configuration
            )
        return this._dataTransformationSetupApi;
    };

    public get datasetSpecification () : DefaultReduxApi<DatasetSpecification,DatasetSpecificationApi> {
        if (!this._datasetSpecificationApi)
            this._datasetSpecificationApi = new DefaultReduxApi<DatasetSpecification,DatasetSpecificationApi> (
    ...
    public get fundingInformation () : DefaultReduxApi<FundingInformation,FundingInformationApi> {
        if (!this._fundingInformationApi)
            this._fundingInformationApi = new DefaultReduxApi<FundingInformation,FundingInformationApi> (
    ...
    public get geoShape () : DefaultReduxApi<GeoShape,GeoShapeApi> {
        if (!this._geoShapeApi)
            this._geoShapeApi = new DefaultReduxApi<GeoShape,GeoShapeApi> (
                GeoShapeApi, this.username, this.configuration
            )
    ...
    public get grid () : DefaultReduxApi<Grid,GridApi> {
        if (!this._gridApi)
            this._gridApi = new DefaultReduxApi<Grid,GridApi> (
                GridApi, this.username, this.configuration
            )
    ...
    public get image () : DefaultReduxApi<Image,ImageApi> {
        if (!this._imageApi)
            this._imageApi = new DefaultReduxApi<Image,ImageApi> (
                ImageApi, this.username, this.configuration
            )
    ...
    public get intervention () : DefaultReduxApi<Intervention,InterventionApi> {
        if (!this._interventionApi)
            this._interventionApi = new DefaultReduxApi<Intervention,InterventionApi> (
    ...
    public get modelCategory () : DefaultReduxApi<ModelCategory,ModelCategoryApi> {
        if (!this._modelCategoryApi)
            this._modelCategoryApi = new DefaultReduxApi<ModelCategory,ModelCategoryApi> (
    ...
    public get numericalIndex () : DefaultReduxApi<NumericalIndex,NumericalIndexApi> {
        if (!this._numericalIndexApi)
            this._numericalIndexApi = new DefaultReduxApi<NumericalIndex,NumericalIndexApi> (
    ...
    public get organization () : DefaultReduxApi<Organization,OrganizationApi> {
        if (!this._organizationApi)
            this._organizationApi = new DefaultReduxApi<Organization,OrganizationApi> (
    ...
    public get person () : DefaultReduxApi<Person,PersonApi> {
        if (!this._personApi)
            this._personApi = new DefaultReduxApi<Person,PersonApi> (
                PersonApi, this.username, this.configuration
            )
    ...
    public get process () : DefaultReduxApi<Process,ProcessApi> {
        if (!this._processApi)
            this._processApi = new DefaultReduxApi<Process,ProcessApi> (
                ProcessApi, this.username, this.configuration
            )
    ...
    public get sampleCollection () : DefaultReduxApi<SampleCollection,SampleCollectionApi> {
        if (!this._sampleCollectionApi)
            this._sampleCollectionApi = new DefaultReduxApi<SampleCollection,SampleCollectionApi> (
    ...
    public get sampleResource () : DefaultReduxApi<SampleResource,SampleResourceApi> {
        if (!this._sampleResourceApi)
            this._sampleResourceApi = new DefaultReduxApi<SampleResource,SampleResourceApi> (
    ...
    public get softwareImage () : DefaultReduxApi<SoftwareImage,SoftwareImageApi> {
        if (!this._softwareImageApi)
            this._softwareImageApi = new DefaultReduxApi<SoftwareImage,SoftwareImageApi> (
    ...
    public get sourceCode () : DefaultReduxApi<SourceCode,SourceCodeApi> {
        if (!this._sourceCodeApi)
            this._sourceCodeApi = new DefaultReduxApi<SourceCode,SourceCodeApi> (
                SourceCodeApi, this.username, this.configuration
            )
    ...
    public get standardVariable () : DefaultReduxApi<StandardVariable,StandardVariableApi> {
        if (!this._standardVariableApi)
            this._standardVariableApi = new DefaultReduxApi<StandardVariable,StandardVariableApi> (
    ...
    public get timeInterval () : DefaultReduxApi<TimeInterval,TimeIntervalApi> {
        if (!this._timeIntervalApi)
            this._timeIntervalApi = new DefaultReduxApi<TimeInterval,TimeIntervalApi> (
    ...
    public get unit () : DefaultReduxApi<Unit,UnitApi> {
        if (!this._unitApi)
            this._unitApi = new DefaultReduxApi<Unit,UnitApi> (
                UnitApi, this.username, this.configuration
            )
        return this._unitApi;
    };

    //public get variable () : DefaultReduxApi<Variable,VariableApi> {};
    public get variablePresentation () : DefaultReduxApi<VariablePresentation,VariablePresentationApi> {
        if (!this._variablePresentationApi)
            this._variablePresentationApi = new DefaultReduxApi<VariablePresentation,VariablePresentationApi> (
    ...
    public get visualization () : DefaultReduxApi<Visualization,VisualizationApi> {
        if (!this._visualizationApi)
            this._visualizationApi = new DefaultReduxApi<Visualization,VisualizationApi> (
