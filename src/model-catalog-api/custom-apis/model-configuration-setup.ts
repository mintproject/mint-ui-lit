...
export class CustomModelConfigurationSetupApi extends DefaultReduxApi<
  ModelConfigurationSetup,
  ModelConfigurationSetupApi
> {
  public constructor(
    ApiType: new (cfg?: Configuration) => ModelConfigurationSetupApi,
    user: string,
  ...
  public post: ActionThunk<Promise<ModelConfigurationSetup>, MCActionAdd> =
    (resource: ModelConfigurationSetup, configid: string) => (dispatch) => {
      return new Promise((resolve, reject) => {
        if (!configid)
          throw "Error creating setup. Invalid parent configuration ID.";
        // Check adjustable parameters.
        if (resource.hasParameter)
          resource.adjustableParameter = resource.hasParameter.filter(
            (p: Parameter) =>
              p && (!p.hasFixedValue || p.hasFixedValue.length === 0)
          );
        let setupPost: Promise<ModelConfigurationSetup> = dispatch(
        ...
        setupPost.then((newSetup: ModelConfigurationSetup) => {
          let parentModelConfigurationGet: Promise<ModelConfiguration> =
            dispatch(
              ModelCatalogApi.myCatalog.modelConfigurationSetup.get(configid)
            );
          ...
          parentModelConfigurationGet.then((config: ModelConfiguration) => {
            ...
            let parentModelConfigurationPut: Promise<ModelConfiguration> =
              dispatch(
                ModelCatalogApi.myCatalog.modelConfiguration.put(config)
              );
  ...
  public getDetailsNoRedux(uri: string): Promise<ModelConfigurationSetup> {
    ...
    return this._api.customModelconfigurationsetupsIdGet({
      username: this._username,
      id: id,
    });
  }

  public getSetupsByVariableLabel(
    variableLabel: string
  ): Promise<ModelConfigurationSetup[]> {
    return this._api.customModelconfigurationsetupsVariableGet({
      username: this._username,
      label: variableLabel,
    });
  }
