...
export class CustomRegionApi extends DefaultReduxApi<Region, RegionApi> {
  public constructor(
    ApiType: new (cfg?: Configuration) => RegionApi,
    user: string,
  ...
  public post: ActionThunk<Promise<Region>, MCActionAdd> =
    (resource: Region) => (dispatch) => {
      return new Promise((resolve, reject) => {
        // Create geoShape first
        if (resource.geo && resource.geo.length > 0) {
          let geoShapePost: Promise<GeoShape> = dispatch(
            ModelCatalogApi.myCatalog.geoShape.post(resource.geo[0])
          );
          geoShapePost.catch(reject);
          geoShapePost.then((newGeo: GeoShape) => {
            resource.geo = [newGeo];
            // Then create region
            let regionPost: Promise<Region> = dispatch(
