...
export class CustomParameterApi extends DefaultReduxApi<
  Parameter,
  ParameterApi
> {
  public constructor(
    ApiType: new (cfg?: Configuration) => ParameterApi,
    user: string,
  ...
  private _fixRecommendedIncrement(p: Parameter): Parameter {
    if (p.recommendedIncrement && p.recommendedIncrement.length > 0) {
      //Should only be one
      if (
        typeof p.recommendedIncrement[0] === "string" ||
        (p.recommendedIncrement[0] as any) instanceof String
      ) {
        console.log("Fixed recommended increment", p.recommendedIncrement[0]);
        p.recommendedIncrement = [
          parseFloat(String(p.recommendedIncrement[0])),
        ];
      }
    }
    return p;
  ...
  private simpleGetAll: ActionThunk<Promise<IdMap<Parameter>>, MCActionAdd> =
    this.getAll;
  ...
  public get: ActionThunk<Promise<Parameter>, MCActionAdd> =
    (uri: string) => (dispatch) => {
  ...
  public getAll: ActionThunk<Promise<IdMap<Parameter>>, MCActionAdd> =
      ...
      return new Promise((resolve, reject) => {
        ...
        pReq.then((parameters: IdMap<Parameter>) => {
          Object.keys(parameters).forEach((paramid: string) => {
            parameters[paramid] = this._fixRecommendedIncrement(
              parameters[paramid]
            );
          });
          dispatch({
  ...
  public post: ActionThunk<Promise<Parameter>, MCActionAdd> =
    (resource: Parameter) => (dispatch) => {
      return new Promise((resolve, reject) => {
  ...
  public put: ActionThunk<Promise<Parameter>, MCActionAdd> =
    (resource: Parameter) => (dispatch) => {
      return new Promise((resolve, reject) => {
        resource = this._fixRecommendedIncrement(resource);
