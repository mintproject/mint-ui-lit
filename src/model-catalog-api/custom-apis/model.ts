...
export class CustomModelApi extends DefaultReduxApi<Model, ModelApi> {
  ...
  public isCoupledModel: (t: string) => boolean = (t: string) =>
    t === "https://w3id.org/okn/o/sdm#CoupledModel" || t === "CoupledModel";

  public constructor(
    ApiType: new (cfg?: Configuration) => ModelApi,
    user: string,
  ...
  public get: ActionThunk<Promise<Model | CoupledModel>, MCActionAdd> =
    (uri: string) => (dispatch) => {
      let id: string = this._getIdFromUri(uri);
      return new Promise((resolve, reject) => {
        let req: Promise<Model> = this._api.modelsIdGet({
        ...
        req.then((resp: Model) => {
          if (resp.type.some(this.isCoupledModel)) {
            //Is coupled model
            let req2: Promise<CoupledModel> =
              this._coupledApi.coupledmodelsIdGet({
                username: this._username,
                id: id,
              });
  ...
  public getAll: ActionThunk<
    Promise<IdMap<Model | CoupledModel>>,
    MCActionAdd
  > =
    ...
    (dispatch) => {
      if (ignoreCache || !this._cached) {
        this._cached = new Promise((resolve, reject) => {
          ...
          let reqCoupled: Promise<CoupledModel[]> =
            this._coupledApi.coupledmodelsGet({
              username: this._username,
              perPage: 200,
            });
          let allProm: [Promise<Model[]>, Promise<CoupledModel[]>] = [
            req,
            reqCoupled,
          ];
          ...
          allReq.then(([models, coupledModels]) => {
            ...
            let idCoupledModels: IdMap<CoupledModel> = coupledModels.reduce(
              this._idReducer,
              {}
            );
            let both: IdMap<Model | CoupledModel> = {
              ...idModels,
              ...idCoupledModels,
            };
            if (this._redux)
  ...
  public delete: ActionThunk<Promise<void>, MCActionDelete> =
    (uri: string) => (dispatch) => {
      ...
      let id: string = this._getIdFromUri(uri);
      let req: Promise<void> = this._api.modelsIdDelete({
        user: this._username,
        id: id,
      });
      if (this._redux)
  ...
  public put: ActionThunk<Promise<Model | CoupledModel>, MCActionAdd> =
    (resource: Model | CoupledModel) => (dispatch) => {
      if (!this.isEditable()) return this._notAllowedError();
  ...
  public getModelsByIndexLabel(indexLabel: string): Promise<Model[]> {
    return this._api.customModelIndexGet({
      username: this._username,
      label: indexLabel,
    });
  }
  ...
  public getModelsByStandardVariableLabel(svLabel: string): Promise<Model[]> {
    return this._api.customModelsStandardVariableGet({
      username: this._username,
      label: svLabel,
    });
  }

  public getModelsByInterventionLabel(
    interventionLabel: string
  ): Promise<Model[]> {
    return this._api.customModelInterventionGet({
      username: this._username,
  ...
  public getModelsByRegionLabel(regionLabel: string): Promise<Model[]> {
    return this._api.customModelRegionGet({
      username: this._username,
      label: regionLabel,
    });
  }
