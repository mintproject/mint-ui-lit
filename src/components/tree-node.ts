import { TemplateResult, customElement, LitElement, property, html, css } from "lit-element";
...
export class TreeNode extends LitElement {
    ...
    protected render() {
        ...
        return html`
            <div class="node">
                <span class="title ${this.onClick? 'clickable' : ''} ${this._selected? 'selected' : ''}" @click=${this.onClick}>
                    ${this.tagIcon ? html`<wl-icon style="width: 20px;">${this.tagIcon}</wl-icon>` : (
                            this.tag ? html`<span class="tag">${this.tag}</span>` : '')}
                    ${this.getName()}
                </span>
                ${this.selectIcon && this._selected ? html`<wl-icon class="select-icon">check</wl-icon>` : ''}
                <wl-icon class="expand-icon no-select ${isEmpty ? 'empty' : ''}" @click="${isEmpty ? this.onClick : this.toggle}">
                    ${isEmpty ? 'fiber_manual_record' : (
    ...
    public getFilteredChilds () {
        return this._childs.filter((c:TreeNode) => c.name.toLowerCase().includes(this.textFilter));
    }
