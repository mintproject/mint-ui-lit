...
export class GoogleMapCustom extends GoogleMap {
  ...
  public addRegion(region: Region) {
    let features = this.map.data.addGeoJson(
      geometriesToGeoJson(region.geometries)
    );
    // TODO: Add click handlers for the region
    this.setRegionFeatureProperties(region, features);
  }

  private setRegionFeatureProperties(
    region: Region,
  ...
  private setEventHandlers() {
    ...
    this.map.data.addListener("click", function (event) {
      ...
      map.infoWindow.setContent(event.feature.getProperty("region_name")),
        map.infoWindow.setPosition(
          event.feature.getProperty("center") || event.latlng
        );
  ...
  public alignMapToBoundingBoxes(bboxes: BoundingBox[]) {
    let regions = bboxes.map((bbox) => {
      return { bounding_box: bbox } as Region;
    });
    this.alignMapToRegions(regions);
  }
  ...
  public alignMapToPoints(points: Point[]) {
    let rect = this.getDimensions();
    let midpoint = calculateMapDetailsForPoints(
      points,
      rect.width,
      rect.height
    );
