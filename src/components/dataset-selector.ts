...
export class DatasetSelector extends connect(store)(LitElement) {
  ...
  public setDatasets(
    ...
    datasets.forEach((ds: Dataset) => {
      let newOpt: DatasetOption = {
        value: ds,
        isSelected:
          this.asyncSelectedDatasets && this.asyncSelectedDatasets.has(ds.id),
      };
  ...
  public setSelected(selectedIds: Set<string>): void {
    if (this.loadingData) {
      ...
      this.options.forEach(
        (opt: DatasetOption) => (opt.isSelected = selectedIds.has(opt.value.id))
      );
      this.requestUpdate();
    }
  }
  ...
  private applyTextFilter(options: DatasetOption[]): DatasetOption[] {
    if (this.textFilter)
      return options.filter((opt: DatasetOption) => {
        return (
          opt.value.name.toLowerCase().includes(this.textFilter) ||
          (opt.value.description &&
            opt.value.description.toLowerCase().includes(this.textFilter))
        );
      });
    return options;
  }
  ...
  private renderMatchingDatasets(
    posibleOptions: DatasetOption[]
  ): TemplateResult {
    ...
    let selectedOptions: DatasetOption[] = this.options.filter(
      (opt: DatasetOption) => {
        return opt.isSelected;
      }
    );
  ...
  private renderCategorySeparator(catId: string): TemplateResult {
    return html` <tr>
      <td
        colspan="5"
        style="font-weight: bold; font-size: 1.02em; padding-left: 2em;"
      >
    ...
    return html`
        <tr>
            ...
            <td>
                ...
                }/datasets/browse/${dataset.id}/${this.selectedRegion.id}">${
      dataset.name
    }</a>
                ${
                  dataset.description
            ...
            <td>
                ${
                  dataset.datatype
                    ? html`<div><code>${dataset.datatype}</code></div>`
                    : ""
                }
                ${
                  dataset.region
                    ? html`<div><b>${dataset.region}</b></div>`
                    : ""
                }
                ${
                  dataset.categories
                    ? html`<div>${dataset.categories.join(", ")}</div>`
                    : ""
                }
                ${
                  dataset.time_period
                    ? html`<div>
                        ${dataset.time_period.start_date
                          ? "from " +
                            toDateString(dataset.time_period.start_date)
                          : ""}
                        ${dataset.time_period.end_date
                          ? " to " + toDateString(dataset.time_period.end_date)
                          : ""}
                      </div>`
                    : ""
                }
            ...
            <td>
                ${
                  dataset.source
                    ? html`
                        <a href="${dataset.source.url}" target="_blank"
