...
export class EditableNote extends LitElement {
    ...
    protected render() {
        ...
        return html`
            <fieldset>
                <legend>Notes</legend>
                <textarea id="editable-text" @keyup="${this._autoGrow}">${this.text}</textarea>
                <div id="footer">
                    <wl-button inverted @click="${this._cancel}"
                        style="${edited? 'visibility: visible; opacity:1;' : 'visibility: hidden; opacity: 0;'}"
                        >DISCARD CHANGES</wl-button>
                    <wl-button type="button" class="submit" @click="${this._save}" ?disabled=${!edited}
                        style="${edited? 'visibility: visible; opacity:1;' : ''}">SAVE</wl-button>
                </div>
            </fieldset>
        `;
    }
