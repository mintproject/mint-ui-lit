...
import { ModelMap, ModelEnsembleMap, ComparisonFeature, StepUpdateInformation } from "../reducers/mint";
...
export class MintModels extends connect(store)(MintPathwayPage) {
    ...
    private _comparisonFeatures: Array<ComparisonFeature> = [
        ...
        },        
        {
            ...
            fn: (model:Model) => model.input_parameters.map((ip) => { return ip.name; }).join(", ")
        },
        {
            name: "Model type",
