...
export class RegionSelector extends LitElement {
  ...
  public cancel() {
    ...
    if (this.selectedRegion) {
      this.selectedCategoryId = this.selectedRegion.category_id
        ? this.selectedRegion.category_id
        : "";
    }
  }
  ...
  categoryMatcher = (reg: LocalRegion, id: string) =>
    reg.category_id.toLowerCase() === id.toLowerCase();

  public renderEditForm(): TemplateResult {
    let nRegions: IdMap<number> = {};
    Object.values(this.categories || {}).forEach((cat: RegionCategory) => {
      nRegions[cat.id] = this.mapRegions.filter((r) =>
        this.categoryMatcher(r, cat.id)
      ).length;
      cat.subcategories.forEach((subcat: RegionCategory) => {
        nRegions[subcat.id] = this.mapRegions.filter((region: LocalRegion) =>
          this.categoryMatcher(region, subcat.id)
        ).length;
    ...
    return html`
      <form id="regionForm">
        ...
        <select
          ...
          value=""
          @change="${this.onRegionCategoryChange}"
        >
          <option value="">None</option>
          ${Object.values(this.categories || {})
            .filter((cat: RegionCategory) => nRegions[cat.id] > 0)
            .map((cat: RegionCategory) => {
              let subCategories: RegionCategory[] = cat.subcategories;
              return html` <option value="${cat.id}">${cat.name}</option>
                ${subCategories.length > 0
                  ? subCategories
                      .filter(
                        (subcat: RegionCategory) => nRegions[subcat.id] > 0
                      )
                      .map((subcat: RegionCategory) => {
                        return html`<option
                          value="${subcat.id}"
                          ?selected=${subcat.id == this.selectedCategoryId}
                        >
                          &nbsp;&nbsp;&nbsp;&nbsp;${subcat.name}
                        </option>`;
                      })
      ...
      <div style="width: fit-content;">
        <google-map-custom
          ...
          id="editable-map"
          .style="height: ${this.mapReady ? "400px" : "0px"}; visibility: ${this
            .mapReady
            ? "visible"
            : "hidden"};"
      ...
      ${this.mapReady && this.selectedRegion
        ? html` <div class="mapFooter">
            <div>
              ${this.selectedRegion
                ? html`
                    <b>Selected region name:</b> ${this.selectedRegion.name}
                    <br />
                    <b>Bounding Box:</b>
                    ${this.selectedRegion.bounding_box.xmin.toFixed(4) +
                    "," +
                    this.selectedRegion.bounding_box.ymin.toFixed(4)}
                    ${this.selectedRegion.bounding_box.xmax.toFixed(4) +
                    "," +
                    this.selectedRegion.bounding_box.ymax.toFixed(4)}
                  `
                : ""}
            </div>
          </div>`
  ...
  private handleMapClick(ev: any): void {
    if (ev.detail && ev.detail.id) this.candidateId = ev.detail.id;
    //console.log("-->", ev.detail.id);
  }

  public renderView(): TemplateResult {
    return html`${!this.selectedRegionId
      ? "None selected"
      : !this.selectedRegion
      ...
      <wl-dialog id="regionDialog" fixed backdrop blockscrolling>
        ...
        <div slot="content" style="width: fit-content;">
          <google-map-custom
            ...
            api-key="${GOOGLE_API_KEY}"
            id="non-editable-map"
            .style="height: ${this.mapReady
              ? "400px"
              : "0px"}; visibility: ${this.mapReady
              ? "visible"
  ...
  private onRegionCategoryChange() {
    let form: HTMLFormElement =
      this.shadowRoot!.querySelector<HTMLFormElement>("#regionForm")!;
    let categoryId = (form.elements["category-selector"] as HTMLSelectElement)
      .value;
  ...
  public addRegionsToMap() {
    ...
    if (this.editMode)
      ...
      map = this.shadowRoot.querySelector(
        "#non-editable-map"
      ) as GoogleMapCustom;

    let selectedRegion: string = this.selectedRegionId
      ? this.selectedRegionId
      : this.topRegionId;
    ...
    if (this.topRegionId === selectedRegion && !this.selectedCategoryId) {
      ...
      if (this.selectedCategoryId) {
        visibleRegions = this.mapRegions.filter(
          (region: LocalRegion) => region.category_id == this.selectedCategoryId
        );
      } else {
        //Search for Id on all regions.
        let candidates: LocalRegion[] = this.mapRegions.filter(
          (region: LocalRegion) => region.id == selectedRegion
        );
        if (candidates.length > 0) {
          ...
          visibleRegions = this.mapRegions.filter(
            (region: LocalRegion) =>
              region.category_id == this.selectedCategoryId
          );
        }
      }
    }
  ...
  public loadRegionState(state: RegionsState, parentRegionId: string) {
    ...
    if (this.topRegionId && subRegionIds && subRegionIds[this.topRegionId]) {
      this.mapRegions = subRegionIds[this.topRegionId].map(
        (regionid: string) => state.regions[regionid]
      );
    }
