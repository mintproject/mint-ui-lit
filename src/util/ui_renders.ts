...
export const renderVariables = (
  ...
  driving_callback: Function
) => {
  return html`
    <p>
      Indicators are the variables or index that indicates the state of the
      system being modeled. Adjustable parameters are the variables and
      interventions you are interested in changing to explore outcomes on the
      indicator.
    </p>
    <div class="input_full">
      <label
        >Indicators/Response of interest
        <span style="font-family: auto;">*</span></label
      >
...
export const renderNotifications = () => {
  return html`
    <wl-snackbar
      id="formValuesIncompleteNotification"
      fixed
      backdrop
      disableFocusTrap
    >
...
const varmodel = {
  ...
  atmosphere_water__precipitation_evapotranspiration_standardized_index:
    "Drought Indices",
  atmosphere_water__precipitation_standardized_index:
    "Drought Indices, DroughtPredict",
...
export const renderResponseVariables = (
  variableid: string,
  variables: VariableMap,
  readonly: boolean,
  callback: Function
  ...
  Object.values(variables)
    .filter((varobj) => varobj.is_indicator)
    .map((varobj) => {
...
export const renderDrivingVariables = (
  variableid: string,
  variables: VariableMap,
  readonly: boolean,
  callback: Function
  ...
  Object.values(variables)
    .filter((varobj) => varobj.is_adjustment_variable)
    .map((varobj) => {
      (varobj.categories || []).map((category) => {
  ...
  return html`
    <select name="driving_variable" ?disabled="${readonly}" @change=${callback}>
      ...
      ${Object.keys(adjustmentsByCategory)
        .sort()
        .map((categoryname) => {
...
export const renderLastUpdateText = (info: MintEvent) => {
  ...
  return html`
    <div class="information">
      Last updated by ${info.userid} on ${date.toString()}
    </div>
  `;
};
...
export const urlify = (text: string) => {
  ...
  parts.forEach((p: string) => {
    if (p.match(urlRegex)) {
      ...
      htmls.push(
        html`<a target="_blank" href="${p}">${p}</a>${endp ? "." : ""}`
      );
    } else {
      htmls.push(p);
    }
