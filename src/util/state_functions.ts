...
export const getThreadConfigureStatus = (thread: Thread) => {
  if (
    getThreadModelsStatus(thread) === TASK_DONE &&
    getThreadDatasetsStatus(thread) === TASK_DONE
  )
    return TASK_DONE;
  if (
    !thread.regionid ||
    (thread.response_variables && thread.response_variables.length === 0)
  )
...
export const getThreadRunsStatus = (thread: Thread) => {
  ...
  if (sum && Object.keys(sum).length > 0) {
    ...
    Object.keys(sum).map((modelid) => {
      ...
      if (!summary.total_runs || summary.successful_runs != summary.total_runs)
        ok = false;
    });
    if (ok) return TASK_DONE;
  }
  return TASK_NOT_STARTED;
};

export const getThreadResultsStatus = (thread: Thread) => {
  if (getThreadRunsStatus(thread) != TASK_DONE) return TASK_NOT_STARTED;
  ...
  if (sum && Object.keys(sum).length > 0) {
    ...
    Object.keys(sum).map((modelid) => {
      let summary = sum[modelid];
      if (!summary.total_runs || summary.ingested_runs != summary.total_runs)
        ok = false;
    });
    if (ok) return TASK_DONE;
  }
...
export const getUISelectedProblemStatement = (state: RootState) => {
  if (state.modeling && state.modeling.problem_statements) {
    if (state.ui && state.ui.selected_problem_statement_id) {
      return state.modeling.problem_statements[
        state.ui.selected_problem_statement_id
      ];
    }
  }
  return null;
...
export const getUISelectedThread = (state: RootState) => {
  if (state.modeling && state.modeling.problem_statement) {
    if (
      ...
      let task =
        state.modeling.problem_statement.tasks[state.ui.selected_task_id];
...
export const getVisualizationURLs = (
  thread: Thread,
  task: Task,
  ...
  if (getThreadResultsStatus(thread) == "TASK_DONE") {
    let responseV =
      thread.response_variables.length > 0 ? thread.response_variables[0] : "";
    ...
    if (responseV == "flooding_contour") {
      ...
      responseV == "channel~stream_water__flow_duration_index" ||
...
export const getCategorizedRegions = (state: RootState) => {
  if (
    ...
    let all_regionids =
      state.regions.sub_region_ids[state.ui.selected_top_regionid];
...
const _datasetUsedInOtherModel = (
  thread: Thread,
  datasetid: string,
  notmodelid: string
) => {
  ...
  for (modelid in thread.model_ensembles) {
    if (modelid != notmodelid) {
      let data_ensembles: ModelIOBindings =
        thread.model_ensembles![modelid].bindings;
