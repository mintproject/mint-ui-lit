...
export const regionFromGQL = (regionobj: any): Region => {
  let region = {
    ...
    geometries: (regionobj.geometries ?? []).map(
...
export const problemStatementToGQL = (
...
export const problemStatementUpdateToGQL = (
  problem_statement: ProblemStatementInfo
) => {
  let problemobj = {
...
export const taskFromGQL = (task: any): Task => {
  let taskobj = {
    ...
    threads: {},
    driving_variables:
      task.driving_variable_id != null ? [task.driving_variable_id] : [],
    response_variables:
      task.response_variable_id != null ? [task.response_variable_id] : [],
  ...
  let threadobj = {
    ...
    response_variable_id:
      thread.response_variables && thread.response_variables.length > 0
        ? thread.response_variables[0]
        : null,
    driving_variable_id:
      thread.driving_variables && thread.driving_variables.length > 0
        ? thread.driving_variables[0]
        : null,
    events: {
...
export const threadInfoFromGQL = (thread: any) => {
  return {
    ...
    driving_variables:
      thread.driving_variable_id != null ? [thread.driving_variable_id] : [],
    response_variables:
...
export const threadFromGQL = (thread: any) => {
  ...
  thread["thread_models"].forEach((tm: any) => {
    ...
    let model_ensemble = modelEnsembleFromGQL(
      tm["data_bindings"],
      tm["parameter_bindings"]
    );
    ...
    (tm["execution_summary"] ?? []).forEach((tmex) => {
      fbthread.execution_summary[model.id] =
        threadModelExecutionSummaryFromGQL(tmex);
...
export const modelEnsembleFromGQL = (
  dbs: any[],
  pbs: any[]
): ModelIOBindings => {
...
export const executionFromGQL = (ex: any, emulator = false): Execution => {
  ...
  ex.parameter_bindings.forEach((param: any) => {
    exobj.bindings[
      emulator ? param.model_parameter.name : param.model_parameter_id
...
const modelParameterToGQL = (input: ModelParameter) => {
  ...
  if ("min" in input && input["min"] != null) input["min"] = input["min"] + "";
  if ("max" in input && input["max"] != null) input["max"] = input["max"] + "";
