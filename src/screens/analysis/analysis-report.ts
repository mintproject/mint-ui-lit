...
export class AnalysisReport extends connect(store)(PageViewElement) {
  ...
  private _renderReportList() {
    return html` <span id="start"></span>
      ${Object.values(this._list.problem_statements).map(
        (ps: ProblemStatementInfo) => html` <wl-expansion
          name="problem_statements"
          @click="${() => this._getTasks(ps)}"
          style="overflow-y: hidden;"
        >
          <span slot="title" style="flex-grow: unset;"> ${ps.name} </span>
          <!--span slot="description" style="overflow: hidden; text-overflow: ellipsis;"-->
          ${this._tasks[ps.id]
            ? Object.values(this._tasks[ps.id]).map((task: Task) =>
                this._renderTask(ps, task)
              )
            : this._renderLoading()}
        </wl-expansion>`
      )}`;
        ...
        (t: ThreadInfo) => html`
          ...
          >
            <wl-title level="4" style="margin: 0">
              ${task.name ? task.name + " - " : ""}
              ${t.name ? t.name : "Default thread"}
            </wl-title>
            ${this._getThreadInfoSummaryText(t)}
            <div slot="after" style="display:flex">${this._renderDates(t)}</div>
          </wl-list-item>
        `
      )}
    `;
  ...
  private _renderReport() {
    ...
    if (
      !this._tasks[this._selectedProblemStatementId] &&
      !this._loading[this._selectedProblemStatementId]
    ) {
      this._getTasks(ps);
    }
    if (this._loading[this._selectedProblemStatementId])
      return this._renderLoading();
    let task =
      this._tasks[this._selectedProblemStatementId][this._selectedTaskId];
    ...
    return html`
      <div class="main-content">
        <wl-title level="2" class="two-column-grid" style="padding: 0px;">
          ...
          <span
            >${task.name ? task.name + " - " : ""}
            ${thread.name ? thread.name : "Default thread"}</span
          >
        </wl-title>
      ...
      <div class="two-column-grid inner-content">
        ...
        <span>
          ${!task.response_variables || task.response_variables.length == 0
            ? "No indicators"
            : task.response_variables.map(
                (rv) => html` <div>
                  ${this._variableMap[rv]?.name ?? ""} (<span class="monospaced"
                    >${rv}</span
                  >)
                </div>`
              )}
        ...
        <span>
          ${!task.driving_variables || task.driving_variables.length == 0
            ...
            : task.driving_variables.map(
                (dv) => html` <div>
                  ${this._variableMap[dv]?.name ?? ""} (<span class="monospaced"
                    >${dv}</span
                  >)
                </div>`
              )}
      ...
      <div class="inner-content">
        ${!thread.models || Object.keys(thread.models).length == 0
          ...
          : Object.values(thread.models).map((model: Model) => {
              ...
              return html`<a href="${modelurl}" target="_blank"
                >${model.name}</a
              >`;
            })}
        ${notes["SELECT_MODELS"]
  ...
  private _renderParametersSection(thread: Thread, notes: IdMap<string>) {
    return html`
      ...
      <div class="inner-content">
        ${!thread.model_ensembles ||
        Object.keys(thread.model_ensembles).length == 0
          ? "No adjustable variables for this model"
          : Object.values(thread.models).map(
              (model: Model) => html`
                ...
                >
                  ...
                  <tbody>
                    ${Object.keys(
                      thread.model_ensembles[model.id].bindings
                    ).map(
                      (key: string) => html`
                        <tr>
                          <td>${key.split("/").pop()}</td>
        ...
        ${Object.values(thread.models).map(
          (model: Model) => html`
            ...
            ${!thread.execution_summary[model.id]
              ...
              : html`
                  The model setup created
                  ${thread.execution_summary[model.id].total_runs}
                  configurations.
                  ${thread.execution_summary[model.id].submitted_runs} model
                  runs were submitted, out of which
                  ${thread.execution_summary[model.id].successful_runs}
                  succeeded, and
                  ${thread.execution_summary[model.id].failed_runs} failed.
                `}
          `
        )}
      </div>
  ...
  private _getThreadInfoSummaryText(t: ThreadInfo) {
    let response = t.response_variables
      ? this._variableMap[t.response_variables[0]]
      : null;
    let regionname =
      t.regionid && this._regions && this._regions[t.regionid]
        ? this._regions[t.regionid].name
        : this._region.name;
  ...
  stateChanged(state: RootState) {
    ...
    if (state.analysis) {
      ...
      if (
        state.analysis.problem_statement &&
        !this._tasks[state.analysis.problem_statement.id]
      ) {
        this._tasks[state.analysis.problem_statement.id] =
          state.analysis.problem_statement.tasks;
        this._loading[state.analysis.problem_statement.id] = false;
        this.requestUpdate();
      }
      if (
        state.analysis.thread &&
        !this._thread &&
        state.analysis.thread.id === state.ui.selected_thread_id
      ) {
    ...
    if (state.ui) {
      ...
      if (
        state.ui.selected_thread_id &&
        state.ui.selected_thread_id != this._selectedThreadId
      ) {
      ...
      this._selectedThreadId = state.ui.selected_thread_id;
      if (
        state.ui.selected_top_regionid &&
