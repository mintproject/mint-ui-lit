...
export class MessagesThread extends connect(store)(PageViewElement) {
  ...
  render() {
    return html`
      ...
      <link
        rel="stylesheet"
        href="https://unpkg.com/easymde@2.8.0/dist/easymde.min.css"
      />

      <div id="main-grid">
        <div style="grid-row: 1;" class="main-thread">
          ${this._thread
            ? html` <div id="main-post"></div>
                <div>
                  <span class="user">
        ...
        <div style="grid-row: 2; margin-top: 6px;" class="response">
          ...
          ${!this._posts
            ...
            ? html`<div class="info">- No responses yet -</div>`
            : this._posts.map(
  ...
  stateChanged(state: RootState) {
    if (
      ...
      state.ui.selected_thread_id !== this._threadId
    ) {
      // Selected new Thread
    ...
    if (state.messages && state.messages.posts && state.messages.posts.posts) {
      this._posts = Object.values(state.messages.posts.posts).filter(
        (p) => p.threadid === this._threadId
      );
    }
  }
}
