...
export class MessagesHome extends connect(store)(PageViewElement) {
      ...
      <div style="display: grid;">
        <div style="grid-row: 1;">
          ...
          >
            ...
            <wl-button @click="${this._newThread}"
              ><wl-icon>note_add</wl-icon>New topic</wl-button
            >
          </div>
        </div>
        ...
        <wl-list-item
          active
          style="grid-row: 2; display:${this._subpage != "home" ? "none" : ""}"
          class="info"
        >
          <wl-icon slot="before" style="--icon-size: 30px; margin-right: 15px"
            >info</wl-icon
          >
          <span slot="after"></span>
          <wl-title level="4" style="margin: 0">
            Please make sure to check messages for the topics you are interested
            in regularly.
          </wl-title>
          <span>
            This version of the message board will not send alerts when a new
            message is posted.
          </span>
        </wl-list-item>
  ...
  _newThread() {
    let messageList: HTMLInputElement = <HTMLInputElement>(
  ...
  stateChanged(state: RootState) {
    ...
    if (
      ...
      state.messages.threads.threads[state.ui.selected_thread_id]
    ) {
      this._selectedThread =
        state.messages.threads.threads[state.ui.selected_thread_id];
    }
  }
}
