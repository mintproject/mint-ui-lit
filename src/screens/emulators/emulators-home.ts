...
import emulators, {
  EmulatorModelIO,
  EmulatorSearchConstraint,
  EmulatorsList,
  EmulatorsListWithStatus,
} from "./reducers";
import { goToPage } from "app/actions";
import {
  listEmulatorModelTypes,
  listModelTypeInputDataValues,
  listModelTypeInputParamValues,
  listModelTypeIO,
  getNumEmulatorsForFilter,
  searchEmulatorsForModel,
  searchEmulatorsForFilter,
} from "./actions";
...
export class EmulatorsHome extends connect(store)(PageViewElement) {
  ...
  private getInputsHTML(inputs: object) {
    let inames = Object.keys(inputs).sort((a, b) =>
      inputs[a]["from_model"] ? 1 : -1
    );
    ...
    return html`
      <div class="${largeContent ? "largeContent hideContent" : ""}">
        ${inames.map((iname) => {
          let ivalues = inputs[iname]["value"].filter(
            (input: any, index: number, self: any) =>
              index ===
              self.findIndex(
                (t: any) =>
    ...
    if (input) {
      ...
      if (!input.values) {
        if (input.type != "parameter") {
          console.log(
            input.name +
              ": Dispatching input data values for " +
              this._emulatorRegion
          );
          store.dispatch(
            listModelTypeInputDataValues(
  ...
  private _addSearchConstraint() {
    let inputname =
      this.shadowRoot.querySelector<HTMLSelectElement>("#model_input").value;
    let input = this._modelIO.find((input) => input.name == inputname);

    let options = this.shadowRoot.querySelector<HTMLSelectElement>(
      "#model_input_values"
    ).selectedOptions;
    let inputvalues = [];
      ...
      <wl-tab-group align="center" style="width: 100%;">
        ${this._typesLoading
          ...
          : this._modelTypes.map((mtype) => {
              return html` <wl-tab
                @click="${() => this._showEmulators(mtype)}"
                ?checked="${this._selectedModel == mtype}"
                >${mtype}</wl-tab
              >`;
            })}
      </wl-tab-group>
      ...
      <div class="emulators">
        ...
        ${this._typesLoading
          ...
          ? this._modelIO
            ...
            : ""
          : html` <!-- Emulator Executions -->
              <div style="display: flex; align-items: center;">
                <h2 style="padding: 0px 10px;">
                  ${this._selectedModel} Emulators
                </h2>
                <wl-tab-group align="start">
                  <wl-tab
                    @click="${() => (this._tableType = "filter")}"
                    ?checked="${this._tableType === "filter"}"
                  >
                    Filter executions
                  </wl-tab>
                  <wl-tab
                    @click="${() => (this._tableType = "grouped")}"
                    ?checked="${this._tableType === "grouped"}"
                  >
              ...
              ${this._tableType == "filter"
                ? this._modelInputsLoading
                  ...
                  : html` <table
                            ...
                            <td>
                              ...
                              >
                                ...
                                <tbody>
                                  ...
                                  <tr>
                                    <td colspan="3">
                                      Filter model runs based on input parameter
                                      ranges and datasets
                                    </td>
                                  </tr>
                                  <tr>
                                    <td>
                                      <select
                                        id="model_input"
                                        @change="${this._selectConstraintInput}"
                                      >
                                        <option value="">
                                          Select an input
                                        </option>
                      ...
                      <div
                        style="width:100%; border:1px solid #EEE;border-bottom:0px;"
                      >
                        ${this._filtered_emulators &&
                        this._filtered_emulators.total &&
                        !this._filtered_emulators.loading
                          ? html`
                              ${this.currentPage > 1
                                ? html`<wl-button
                                    flat
                                    inverted
                                    @click=${() => this._nextPage(-1)}
                                    >Back</wl-button
                                  >`
                      ...
                      >
                        ...
                        ${!this._filtered_emulators?.list
                          ...
                          ? html`<wl-progress-spinner
                            ...
                            ></wl-progress-spinner>`
                          : html` <table
                              class="pure-table pure-table-striped results_table"
                            >
                              <!-- Heading -->
                              ${inputs.length > 0
                                ? html`<colgroup
                              ...
                              <tbody>
                                ...
                                ${Object.keys(
                                  ...
                                  let ensemble: Execution =
                                    this._filtered_emulators.list[index];
                                  return html`
                                    <tr>
                                      ${inputs.map((input) => {
                ...
                : html` <table class="pure-table pure-table-bordered">
                    <thead>
                      <tr>
                        <th>Model Calibrated for Region</th>
                        <th>Executed for Region</th>
                        <th>Time period</th>
                        <th>Input</th>
                        <th>Model Setup</th>
                        <th>Ensemble description (range of parameters)</th>
                        <th>Output summary (Ensemble)</th>
                      </tr>
                    </thead>
                    <tbody>
                      ${this._emulators.list.map((em) => {
                        ...
                        if (tms && tms.length > 0) {
                          return tms.map((tm) => {
                            ...
                            let numexecutions =
                              tm["executions_aggregate"]["aggregate"]["count"];
                            if (numexecutions > 0) {
  ...
  stateChanged(state: RootState) {
    ...
    if (
      ...
      console.log(
        "Dispatching model types for region: " + this._emulatorRegion
      );
    ...
    if (state.emulators) {
      ...
      if (state.emulators.models && !this._modelTypes) {
        ...
        if (!this._typesLoading) this._modelTypes = state.emulators.models.list;
      }
      // Get executions by thread
      if (
        state.emulators.emulators &&
        state.emulators.emulators[this._selectedModel]
      ) {
        this._emulatorsLoading =
          state.emulators.emulators[this._selectedModel].loading;
      ...
      if (
        state.emulators.filtered_emulators &&
        state.emulators.filtered_emulators[this._selectedModel]
      ) {
        this._filteredEmulatorsLoading =
          state.emulators.filtered_emulators[this._selectedModel].loading;
        if (!this._filteredEmulatorsLoading) {
          let _filtered_emulators =
            state.emulators.filtered_emulators[this._selectedModel];
      ...
      if (
        state.emulators.selected_model &&
        this._modelTypes &&
        !this._dispatched &&
        (!this._selectedModel ||
        ...
        store.dispatch(
          searchEmulatorsForModel(this._selectedModel, this._emulatorRegion)
        );
        store.dispatch(
          listModelTypeIO(this._selectedModel, this._emulatorRegion)
        );
        store.dispatch(
          getNumEmulatorsForFilter(
            this._selectedModel,
      ...
      if (this._selectedModel) {
        ...
        if (
          state.emulators.model_io &&
          state.emulators.model_io[this._selectedModel]
        ) {
          this._modelInputsLoading =
            state.emulators.model_io[this._selectedModel].loading;
