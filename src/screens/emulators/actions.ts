...
export const EMULATORS_LIST_EXECUTIONS_FOR_MODEL =
  "EMULATORS_LIST_EXECUTIONS_FOR_MODEL";
export const EMULATORS_LIST_THREAD_EXECUTIONS_JSON =
  "EMULATORS_LIST_THREAD_EXECUTIONS_JSON";
export const EMULATORS_LIST_MODEL_IO = "EMULATORS_LIST_MODEL_IO";
export const EMULATORS_LIST_MODEL_INPUT_DATA_VALUES =
  "EMULATORS_LIST_MODEL_INPUT_DATA_VALUES";
export const EMULATORS_LIST_MODEL_INPUT_PARAM_VALUES =
  "EMULATORS_LIST_MODEL_INPUT_PARAM_VALUES";
export const EMULATORS_LIST_EXECUTIONS_FOR_FILTER =
  "EMULATORS_LIST_EXECUTIONS_FOR_FILTER";
export const EMULATORS_NUM_EXECUTIONS_FOR_FILTER =
  "EMULATORS_NUM_EXECUTIONS_FOR_FILTER";

export interface EmulatorsActionListModels
  extends Action<"EMULATORS_LIST_MODELS"> {
  loading: boolean;
  models?: string[];
}
export interface EmulatorsActionSelectModel
  extends Action<"EMULATORS_SELECT_MODEL"> {
  selected_model: string;
}
export interface EmulatorsActionListEmulatorsForModel
  extends Action<"EMULATORS_LIST_EXECUTIONS_FOR_MODEL"> {
  model: string;
  loading: boolean;
  emulators?: any[];
}
export interface EmulatorsActionListEmulatorsForFilter
  extends Action<"EMULATORS_LIST_EXECUTIONS_FOR_FILTER"> {
  model: string;
  loading: boolean;
  filtered_emulators?: any[];
}
export interface EmulatorsActionNumEmulatorsForFilter
  extends Action<"EMULATORS_NUM_EXECUTIONS_FOR_FILTER"> {
  model: string;
  loading: boolean;
  num_filtered_emulators?: number;
}
export interface EmulatorsActionListThreadExecutionsJson
  extends Action<"EMULATORS_LIST_THREAD_EXECUTIONS_JSON"> {
  threadid: string;
  loading: boolean;
  json: string;
}
export interface EmulatorsActionListModelIO
  extends Action<"EMULATORS_LIST_MODEL_IO"> {
  model: string;
  loading: boolean;
  inputs?: EmulatorModelIO[];
}
export interface EmulatorsActionListModelInputDataValues
  extends Action<"EMULATORS_LIST_MODEL_INPUT_DATA_VALUES"> {
  model: string;
  input: string;
  loading: boolean;
  values?: DataResource[];
}
export interface EmulatorsActionListModelInputParamValues
  extends Action<"EMULATORS_LIST_MODEL_INPUT_PARAM_VALUES"> {
  model: string;
  input: string;
  loading: boolean;
  values?: any[];
...
export type EmulatorsAction =
  | EmulatorsActionListModels
  | EmulatorsActionListEmulatorsForModel
  | EmulatorsActionListEmulatorsForFilter
  | EmulatorsActionNumEmulatorsForFilter
  | EmulatorsActionSelectModel
  | EmulatorsActionListThreadExecutionsJson
  | EmulatorsActionListModelIO
  | EmulatorsActionListModelInputDataValues
  | EmulatorsActionListModelInputParamValues;

const MODEL_PREFIX = "https://w3id.org/okn/i/mint/";

type ListModelsThunkAction = ThunkAction<
  void,
  RootState,
  undefined,
  EmulatorsActionListModels
>;
export const listEmulatorModelTypes: ActionCreator<ListModelsThunkAction> =
  (regionid) => (dispatch) => {
...
type ListEmulatorsThunkAction = ThunkAction<
  void,
  RootState,
  undefined,
  EmulatorsActionListEmulatorsForModel
...
const getExecutionWhereClauseForFilter = (
  filters: EmulatorSearchConstraint[]
) => {
  let query = " where: {";
  query += 'status: {_eq: "SUCCESS"}\n';
  query +=
    "thread_model_executions: {thread_model: {model: {model_name: {_eq: $modelType}}, thread: {_or: [{region_id: {_eq: $regionId}}, {region: {parent_region_id: {_eq: $regionId}}}]}}}";
  query += "_and: [ ";
  query += filters
    .filter((filter) => filter.inputtype == "parameter")
    .map((filter) => {
      return (
        "{  _or: [ " +
        "{ parameter_bindings: {" +
        ' model_parameter: { name: { _eq: "' +
        filter.input +
        '"} } ' +
        " parameter_value: {_in: [ " +
        filter.values.map((v) => '"' + v + '"') +
        " ]}\n" +
        "}}\n" +
        /*"{ model: { parameters: {" + 
  ...
  query += filters
    .filter((filter) => filter.inputtype == "input")
    .map((filter) => {
      return (
        "{  _or: [ " +
        "{ data_bindings: {" +
...
type NumFilteredEmulatorsThunkAction = ThunkAction<
  void,
  RootState,
  undefined,
  EmulatorsActionNumEmulatorsForFilter
>;
export const getNumEmulatorsForFilter: ActionCreator<
  NumFilteredEmulatorsThunkAction
> =
    ...
    APOLLO_CLIENT.query({
      ...
      if (result.error) {
        ...
        dispatch({
          type: EMULATORS_NUM_EXECUTIONS_FOR_FILTER,
          model: model,
          num_filtered_emulators:
            result.data.execution_aggregate.aggregate.count,
          loading: false,
        });
      }
    });
...
type ListFilteredEmulatorsThunkAction = ThunkAction<
  void,
  RootState,
  undefined,
  EmulatorsActionListEmulatorsForFilter
>;
export const searchEmulatorsForFilter: ActionCreator<
  ListFilteredEmulatorsThunkAction
> =
  (
    model: string,
    regionid: string,
...
export const selectEmulatorModel: ActionCreator<EmulatorsActionSelectModel> = (
  model: string
) => {
  return {
...
type ListExecutionsJsonThunkAction = ThunkAction<
  void,
  RootState,
  undefined,
  EmulatorsActionListThreadExecutionsJson
>;
export const getThreadExecutionsJSON: ActionCreator<
  ListExecutionsJsonThunkAction
...
type ListModelIOThunkAction = ThunkAction<
  void,
  RootState,
  undefined,
  EmulatorsActionListModelIO
...
type ListModelInputParamValuesThunkAction = ThunkAction<
  void,
  RootState,
  undefined,
  EmulatorsActionListModelInputParamValues
>;
export const listModelTypeInputParamValues: ActionCreator<
  ListModelInputParamValuesThunkAction
  ...
  APOLLO_CLIENT.query({
    ...
    if (result.error) {
      ...
      values = values.filter(
        (item: any, pos: number) => item && values.indexOf(item) == pos
      );
...
type ListModelInputDataValuesThunkAction = ThunkAction<
  void,
  RootState,
  undefined,
  EmulatorsActionListModelInputDataValues
>;
export const listModelTypeInputDataValues: ActionCreator<
  ListModelInputDataValuesThunkAction
> = (model: string, input: EmulatorModelIO, regionid: string) => (dispatch) => {
  let APOLLO_CLIENT = GraphQL.instance(KeycloakAdapter.getUser());
  APOLLO_CLIENT.query({
