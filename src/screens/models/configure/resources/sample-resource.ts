...
export class ModelCatalogSampleResource extends connect(store)(
  ...
  protected resourceApi: DefaultReduxApi<SampleResource, BaseAPI> =
    ModelCatalogApi.myCatalog.sampleResource;
  ...
  protected _renderResource(r: SampleResource) {
    return r.value
      ? html`<a target="_blank" href="${r.value[0]}">${getLabel(r)}</a>`
      : html`${getLabel(r)}`;
  }
  ...
  protected _renderForm() {
    ...
    return html` <form>
      ...
      <wl-textfield
        id="dcid"
        label="Data catalog ID"
        value="${edResource && edResource.dataCatalogIdentifier
          ? edResource.dataCatalogIdentifier[0]
          : ""}"
      >
      </wl-textfield>
      <wl-textfield
  ...
  protected _getResourceFromForm() {
    ...
    let inputDCID: Textfield = this.shadowRoot.getElementById(
      "dcid"
    ) as Textfield;
    ...
    if (label && desc && (dcid || value)) {
      ...
      if (dcid) jsonRes["dataCatalogIdentifier"] = [dcid];
      else if (
        ed &&
        ed.dataCatalogIdentifier &&
        ed.dataCatalogIdentifier.length > 0
      )
        jsonRes["dataCatalogIdentifier"] = [];

      if (value) jsonRes["value"] = [value];
      else if (ed && ed.value && ed.value.length > 0) jsonRes["value"] = [];
