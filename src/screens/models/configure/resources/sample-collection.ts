...
export class ModelCatalogSampleCollection extends connect(store)(
  ...
  protected resourceApi: DefaultReduxApi<SampleCollection, BaseAPI> =
    ModelCatalogApi.myCatalog.sampleCollection;
...
export interface SampleCollection {
  ...
  protected _renderResource(r: SampleCollection) {
    return html`
      <div
        style="display: flex; min-width: 200px; justify-content: space-between;"
      >
        <b>${getLabel(r)}</b>
        <span class="monospaced">
          ${r.hasPart
            ? r.hasPart.length + " file" + (r.hasPart.length > 1 ? "s" : "")
            : ""}
        </span>
      </div>
  ...
  protected _getResourceFromForm() {
    ...
    let sampleResources: SampleResource[] =
      this._inputSampleResources.getResources();
