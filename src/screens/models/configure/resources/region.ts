...
export class ModelCatalogRegion extends connect(store)(
  ...
  protected resourceApi: DefaultReduxApi<Region, BaseAPI> =
    ModelCatalogApi.myCatalog.region;
  ...
  constructor() {
    ...
    this._filters.push(
      (r: Region) =>
        isMainRegion(r) ||
        !this._topRegionUri ||
        isSubregion(this._topRegionUri, r)
    );
  }
  ...
  protected _getResourceFromForm() {
    ...
    if (this._tabMap) {
      ...
      let mcRegion: Region[] = Object.values(this._loadedResources).filter(
        (region: Region) => region.label[0] === selected.name
      );
      ...
      let jsonRes = {
        ...
        geo: [
          GeoShapeFromJSON({
            ...
            box: [
              selected.bounding_box.xmin +
                "," +
                selected.bounding_box.ymin +
                " " +
                selected.bounding_box.xmax +
                "," +
                selected.bounding_box.ymax,
            ],
            type: ["GeoShape"],
          }),
        ],
      ...
      <div slot="footer">
        ${this._tabMap
          ? html`
              ...
              <wl-button
                class="submit"
                ?disabled="${this._waiting || !this._selectedMapRegion}"
  ...
  private _renderTabs() {
    return html`
      <wl-tab-group align="center">
        <wl-tab
          ?checked="${!this._tabMap}"
          @click="${() => {
            this._tabMap = false;
          }}"
          >Search Region</wl-tab
        >
        <wl-tab ?checked="${this._tabMap}" @click="${this._enableMap}"
          >Map</wl-tab
        >
      </wl-tab-group>
    `;
  ...
  private _renderMapTab() {
    return html` <form id="regionForm">
        <div class="input_half">
          <label>Region category</label>
          <select
            name="category-selector"
            value=""
            @change="${this._onRegionCategoryChange}"
          >
            ...
            ${Object.values(this.categories).map((cat: RegionCategory) => {
              ...
              return html` <option value="${cat.id}">${cat.name}</option>
                ${subCategories.length > 0
                  ? subCategories.map((subcat: RegionCategory) => {
                      return html`<option value="${subcat.id}">
                        &nbsp;&nbsp;&nbsp;&nbsp;${subcat.name}
                      </option>`;
                    })
                  : html`
      ...
      <google-map-custom
        ...
        .style="height:400px; visibility: ${this._mapReady
          ? "visible"
          : "hidden"}"
      ...
      ${this._selectedMapRegion
        ? html`
            <b>Selected region name:</b> ${this._selectedMapRegion.name} <br />
            <b>Bounding Box:</b>
            ${this._selectedMapRegion.bounding_box.xmin.toFixed(4) +
            "," +
            this._selectedMapRegion.bounding_box.ymin.toFixed(4)}
            ${this._selectedMapRegion.bounding_box.xmax.toFixed(4) +
            "," +
            this._selectedMapRegion.bounding_box.ymax.toFixed(4)}
          `
        : ""}`;
  }
  ...
  private _onRegionCategoryChange() {
    ...
    let category = (form.elements["category-selector"] as HTMLSelectElement)
      .value;
  ...
  public addRegionsToMap() {
    ...
    let visibleRegions = this._mapRegions.filter(
      (region: LocalRegion) => region.category_id == this._selectedCategory
    );
  ...
  private _handleMapClick(ev: any) {
    if (ev.detail && ev.detail.id) {
      this._selectedMapRegion = this._mapRegions.filter(
        (r) => r.id === ev.detail.id
      )[0];
    }
  }
  ...
  stateChanged(state: RootState) {
    if (state.ui && state.regions) {
      ...
      if (
        ...
        state.regions.regions[this.topRegionId]
      ) {
      ...
      if (this.topRegionId && subRegionIds && subRegionIds[this.topRegionId]) {
        this._mapRegions = subRegionIds[this.topRegionId].map(
          (regionid: string) => state.regions.regions[regionid]
        );
      }
