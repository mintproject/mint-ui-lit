...
export class ModelCatalogGrid extends connect(store)(
  ...
  protected resourceApi: DefaultReduxApi<Grid, BaseAPI> =
    ModelCatalogApi.myCatalog.grid;

  protected _renderResource(r: Grid) {
    let additionalTypes: string[] = (r.type || []).filter(
      (s: string) => s != "Grid" && s != "https://w3id.org/okn/o/sdm#Grid"
    );
    return html`
      <div
        class="one-line"
      ...
      ${r.hasSpatialResolution || r.hasDimension || r.hasShape
        ? html`
            <div
              class="one-line"
              style="display: flex; justify-content: space-between;"
            >
              ${r.hasSpatialResolution && r.hasSpatialResolution.length > 0
                ...
                : ""}
              ${r.hasDimension && r.hasDimension.length > 0
                ? html` <span style="margin-right: 10px;">
                ...
                : ""}
              ${r.hasShape && r.hasShape.length > 0
                ? html`
                    <span class="one-line">
  ...
  protected _renderForm() {
    ...
    return html` <form>
      ...
      <wl-textfield
        ...
        value=${edResource && edResource.hasSpatialResolution
          ? edResource.hasSpatialResolution[0]
          : ""}
      >
      </wl-textfield>
      <div class="two-inputs">
        <wl-select
          id="grid-dim"
          label="Dimension"
          value=${edResource && edResource.hasDimension
            ? edResource.hasDimension[0]
            : ""}
        >
        ...
        <wl-select
          id="grid-shape"
          label="Shape"
          value=${edResource && edResource.hasShape
            ? edResource.hasShape[0]
            : ""}
        >
          <option value="" selected>None</option>
  ...
  protected _getResourceFromForm() {
    ...
    let inputSpRes: Textfield = this.shadowRoot.getElementById(
      "grid-spatial-res"
    ) as Textfield;
    let inputDim: Select = this.shadowRoot.getElementById("grid-dim") as Select;
    let inputShape: Select = this.shadowRoot.getElementById(
      "grid-shape"
    ) as Select;
    // VALIDATE
    ...
    if (label && desc) {
      ...
      if (shape) {
        ...
        else if (shape === "Point") jsonRes.type.push("PointBasedGrid");
      }
