...
export class ModelCatalogConstraint extends connect(store)(
  ...
  protected resourceApi: DefaultReduxApi<Constraint, BaseAPI> =
    ModelCatalogApi.myCatalog.constraint;
  ...
  protected _renderForm() {
    ...
    return html` <form>
      ...
      <span style="margin: 10px 0px 3px 0px; display: block; font-size: small;"
        >Rule:</span
      >
      <textarea id="rule" name="Rule" style="width: 100%;" rows="5">
${edResource && edResource.hasRule ? edResource.hasRule : ""}</textarea
      >
  ...
  protected _getResourceFromForm() {
    ...
    let inputRule: Textarea = this.shadowRoot.getElementById(
      "rule"
    ) as Textarea;
