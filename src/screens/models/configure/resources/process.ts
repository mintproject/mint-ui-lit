...
export class ModelCatalogProcess extends connect(store)(
  ...
  protected resourceApi: DefaultReduxApi<Process, BaseAPI> =
    ModelCatalogApi.myCatalog.process;
  ...
  protected _renderForm() {
    ...
    if (edResource && edResource.id != this._lastId) {
      ...
      (edResource.influences || []).forEach(
        (r: Process) => (this._selectedInfluencers[r.id] = true)
      );
      ...
      <fieldset style="border-radius: 5px; border: 2px solid #D9D9D9">
        ...
        ${this._allResourcesLoading
          ...
          : html`<div style="max-height: 300px; overflow-y: scroll">
              ${resourcesToShow.map(
                (r: Process) => html` <span class="${this.classes} list-item">
                  <span
                    class="clickable-area"
                    @click="${() => {
                      this._selectedInfluencers[r.id] =
                        !this._selectedInfluencers[r.id];
                      this.requestUpdate();
                    }}"
                  >
                    ...
                    <span
                      class="${this._selectedInfluencers[r.id] ? "bold" : ""}"
  ...
  protected _getResourceFromForm() {
    ...
    if (label) {
      ...
      if (influencers.length > 0) jsonRes["influences"] = influencers;
