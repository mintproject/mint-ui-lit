...
export class CompareTree extends connect(store)(PageViewElement) {
  ...
  _addToComparison(c: ComparisonEntry) {
    if (
      this._compare.filter((ce: ComparisonEntry) => ce.uri === c.uri).length ===
      0
    ) {
  ...
  protected firstUpdated() {
    ...
    store
      ...
      .then((categories: IdMap<ModelCategory>) => {
        this._categories = categories;
      });
    store
      .dispatch(ModelCatalogApi.myCatalog.model.getAll())
      .then((models: IdMap<Model>) => {
    ...
    store
      ...
      .then((versions: IdMap<SoftwareVersion>) => {
        ...
        Object.keys(versions).forEach(
          (vid: string) => !!this._nodes[vid] && this._nodes[vid].refresh()
        );
      });
    store
      .dispatch(ModelCatalogApi.myCatalog.modelConfiguration.getAll())
      .then((cfgs: IdMap<ModelConfiguration>) => {
        this._loadingConfigs = false;
        this._configs = cfgs;
        Object.keys(cfgs).forEach(
          (cid: string) => !!this._nodes[cid] && this._nodes[cid].refresh()
        );
      });
    ...
    setupReq.then((setups: IdMap<ModelConfigurationSetup>) => {
      this._setups = setups;
      Object.keys(setups).forEach(
        (sid: string) => !!this._nodes[sid] && this._nodes[sid].refresh()
      );
    });
  ...
  private _calculateTree() {
    ...
    Object.values(this._models).forEach((m: Model) => {
      // Model nodes.
      let category: string =
        m.hasModelCategory && m.hasModelCategory.length > 0
          ? getLabel(
              this._categories && this._categories[m.hasModelCategory[0].id]
      ...
      if (this._loadingVersions) {
        ...
        (m.hasVersion || [])
          ...
          .forEach((v: SoftwareVersion) => {
                ...
                if (tag[0] == "preferred") newNode.setTagIcon("start");
                  ...
                  if (tag[0] == "deprecated")
                    newNode.style.cssText =
                      "--tag-background-color: chocolate;";
                  else if (tag[0] == "latest")
                    newNode.style.cssText =
                      "--tag-background-color: forestgreen;";
            ...
            if (!nodeModel.hasNode(nodeVersion))
              nodeModel.addChild(nodeVersion);
            ...
            if (this._loadingConfigs) {
              ...
              (v.hasConfiguration || [])
                ...
                .forEach((c: ModelConfiguration) => {
                  ...
                  if (!nodeVersion.hasNode(nodeConfig))
                    nodeVersion.addChild(nodeConfig);
                  ...
                  if (this._loadingSetups) {
                    ...
                    (c.hasSetup || [])
                      ...
                      .forEach((s: ModelConfigurationSetup) => {
                        ...
                        if (!nodeConfig.hasNode(nodeSetup))
                          nodeConfig.addChild(nodeSetup);
                      });
                  }
                });
            }
