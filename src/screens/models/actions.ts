...
export interface ModelsActionVariablesQuery
  extends Action<"MODELS_VARIABLES_QUERY"> {
  variables: string[];
  models: Model[] | null;
  loading: boolean;
}
...
const parameterToParam = (parameter) => {
  let param: ModelParameter = {
    ...
    min: parameter.hasMinimumAcceptedValue
      ? parameter.hasMinimumAcceptedValue[0]
      : "",
    max: parameter.hasMaximumAcceptedValue
      ? parameter.hasMaximumAcceptedValue[0]
      : "",
    ...
    accepted_values: parameter.hasAcceptedValues
      ? parameter.hasAcceptedValues[0]
      : null,
  };
  if (parameter.hasFixedValue) param.value = parameter.hasFixedValue[0];
...
const fixedToValue = (fx) => {
  if (fx.type.includes("SampleCollection")) {
    ...
    let dataCatalogIdentifier =
      fx.dataCatalogIdentifier && fx.dataCatalogIdentifier.length > 0
...
export const setupToOldModel = (
  ...
  let model: Model = {
    ...
    software_image:
      setup.hasSoftwareImage &&
      setup.hasSoftwareImage[0].id &&
      softwareImages[setup.hasSoftwareImage[0].id]
        ? (softwareImages[setup.hasSoftwareImage[0].id].label || []).join("")
        : "",
    model_type: (setup.type || [])
      .filter(
        (m) => m != "ConfigurationSetup" && m != "ModelConfigurationSetup"
      )
      .join(", ")
      .replace("Model", " Model"),
    parameter_assignment: setup.parameterAssignmentMethod
      ? setup.parameterAssignmentMethod[0]
      : "",
  ...
  if (
    setup.usefulForCalculatingIndex &&
    setup.usefulForCalculatingIndex.length > 0
  ) {
  ...
  if (setup.hasGrid && setup.hasGrid.length > 0) {
    ...
    (model.spatial_grid_type = types.join()),
      (model.spatial_grid_resolution = grid.hasSpatialResolution
        ? grid.hasSpatialResolution[0]
        : "");
  }
  ...
  if (setup.hasParameter)
    model.input_parameters = setup.hasParameter
      .map(parameterToParam)
      .sort(sortByPosition);

  if (setup.hasRegion) model.hasRegion = setup.hasRegion;

  return model;
};

// Query Model Catalog By Output? Variables TODO FIXME
type QueryModelsThunkResult = ThunkAction<
  void,
  RootState,
  undefined,
  ModelsActionVariablesQuery
>;
export const queryModelsByVariables: ActionCreator<QueryModelsThunkResult> =
  (
  ...
  (dispatch) => {
    ...
    Promise.all(
      variables.map((variable: string) =>
        ModelCatalogApi.myCatalog.modelConfigurationSetup.getSetupsByVariableLabel(
          variable
        )
      )
      /*{ FIXME
      ...
      setups = resp.reduce(
        (arr: ModelConfigurationSetup[], r: ModelConfigurationSetup[]) =>
          arr.concat(r),
        []
      );
      dispatch({
...
export const fetchModelsFromCatalog = async (
  ...
  return Promise.all(
    Object.keys(models || {}).map((modelid) =>
      ModelCatalogApi.myCatalog.modelConfigurationSetup.getDetailsNoRedux(
        modelid
      )
    )
  ).then(async (setups) => {
    let fixedModels = setups.map((setup) =>
      setupToOldModel(setup, allSoftwareImages)
    );
    Object.values(fixedModels).forEach((model) => {
      if (model.hasRegion) delete model.hasRegion;
      Object.values(allConfigs).forEach((cfg: ModelConfiguration) => {
        if (
          (cfg.hasSetup || []).some(
            (setup: ModelConfigurationSetup) => setup.id === model.id
          )
        )
        ...
        Object.values(allVersions).forEach((ver: SoftwareVersion) => {
          if (
            (ver.hasConfiguration || []).some(
              (cfg: ModelConfiguration) => cfg.id === model.model_configuration
            )
          )
      ...
      if (model.model_version) {
        Object.values(allModels).forEach((mod: MCModel) => {
          if (
            (mod.hasVersion || []).some(
              (ver: SoftwareVersion) => ver.id === model.model_version
            )
          )
