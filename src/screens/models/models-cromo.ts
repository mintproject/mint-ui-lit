...
export class ModelsCromo extends connect(store)(PageViewElement) {
    ...
    return html`
        ...
        <div style="display:flex">
            <div style="min-width:440px;max-width:440px">
                ...
                <google-map-custom class="map" api-key="${
                  MINT_PREFERENCES.google_maps_key
                }" 
                    .style="visibility: ${
                      this._mapReady ? "visible" : "hidden"
                    }; display: ${this._mapEmpty ? "unset" : "block"}"
                ...
                ${
                  this._mapReady && this._mapEmpty
                    ? html`
                        <div class="input_full">
                          Please
                          <a @click="${this._showAddRegionsDialog}"
                            >Load a geojson</a
                          >
                ...
                <wl-button @click="${
                  this._onSubmitToCromo
                }" class="submit" id="dialog-submit-button">Search Models</wl-button>
            </div>        
            ...
            <div style="padding-left:20px; width: 100%;">
                ...
                ${(this._modelconfigs || [])
                  .sort((c1, c2) => this.sortConfig(c1, c2))
                  .map((c) => this.renderConfigurationResult(c))}
  ...
  private sortConfig(c1, c2) {
    ...
    let recommended1: boolean =
      c1 &&
      c1.combos &&
      c1.combos.length > 0 &&
      c1.combos.some((cb) => cb.validity.recommended == true);
    let recommended2: boolean =
      c2 &&
      c2.combos &&
      c2.combos.length > 0 &&
  ...
  private renderConfigurationResult(config) {
    let valid: boolean =
      ...
      config.combos.some(
        (cb) => cb.validity.valid == true || cb.validity.valid == null
      );
    let invalid: boolean =
      config &&
      config.combos &&
    ...
    return html`
      <wl-expansion
        name="${config.combos?.length > 0 ? "ok" : "notok"}"
      ...
      >
        ...
        <span slot="description" style="position: absolute; right: 60px;">
          ${config.waiting
            ? html`<wl-progress-spinner
                class="loading small-loading"
              ></wl-progress-spinner>`
            : config.combos === undefined
            ? html`<wl-progress-spinner
                class="waiting small-loading"
              ></wl-progress-spinner>`
            : config.combos.length == 0
        ...
        ${config.combos && config.combos.length == 0
          ? html`
              <p>
                No datasets match this configuration with the current
                parameters.
              </p>
            `
          : ""}
        ${(config.combos || []).map(
          (combo) => html` <p>
              <b>Input Datasets</b> for this model for the selected region and
              time period:
            </p>
            <ul>
              ${combo.inputs.map((input) => {
                ...
                return html` <li>
                  Input <b>${input.input_id}</b> has data
                  "${input.dataset.dataset_name}", with the following
            ...
            <div>
              <h4
                .style="${combo.validity.recommended == true
                  ? "color:green"
                  : "color:gray"}"
              >
              ...
              <ul style="list-style-type: none">
                ${combo.validity?.recommendation_reasons?.length > 0
                  ? html` ${combo.validity.recommendation_reasons.map(
                      (reason) =>
                        html`
                          <li style="color:green">
                            <wl-icon style="--icon-size: 14px;color:green"
                              >done_all</wl-icon
                            >
                            &nbsp; ${reason}
                          </li>
                  ...
                  : ""}
                ${combo.validity?.non_recommendation_reasons?.length > 0
                  ? html` ${combo.validity.non_recommendation_reasons.map(
                      (reason) =>
                        html`
                          <li style="color:grey">
                            <wl-icon style="--icon-size: 14px;color:gray"
                              >close</wl-icon
                            >
                            &nbsp; ${reason}
                          </li>
              ...
              <h4
                .style="${combo.validity.valid == true
                  ? "color:green"
                  : "color:red"}"
              >
              ...
              <ul>
                ${combo.validity?.validity_reasons?.length > 0
                  ? html` ${combo.validity.validity_reasons.map(
                      (reason) =>
                        html`
                          <li style="color:grey">
                            <wl-icon style="--icon-size: 14px;color:green"
                  ...
                  : ""}
                ${combo.validity?.invalidity_reasons?.length > 0
                  ? html` ${combo.validity.invalidity_reasons.map(
                      (reason) =>
                        html`
                          <li style="color:red">
                            <wl-icon style="--icon-size: 14px;color:red"
                              >close</wl-icon
                            >
                            &nbsp; ${reason}
                          </li>
              ...
              >
                <wl-button
                  ?disabled="${this._dummyRunning}"
                  @click="${() => this._dummyRun()}"
                  >${this._dummyRunning ? "Running..." : "Select"}</wl-button
                >
              </div>
            </div>`
        )}
  ...
  async _onSubmitToCromo() {
    let form: HTMLFormElement =
      this.shadowRoot!.querySelector<HTMLFormElement>("#cromo_form");
    ...
    let response = await fetch(
      this.prefs.mint.cromo_api + "/searchModels/" + scenario,
      { method: "post" }
    );
    ...
    for (let i = 0; i < this._modelconfigs.length; i++) {
      ...
      let response = await fetch(
        this.prefs.mint.cromo_api + "/validity/" + cname,
        {
          method: "post",
      ...
      } /*if (response.status == 204) TODO: some request fails, assuming empty */ else
        config.combos = [];
      this.requestUpdate();
    }
  }
    ...
    return html`
        <wl-dialog class="larger" id="addRegionDialog" fixed backdrop blockscrolling>
            ...
            <div slot="footer">
                ...
                }" class="submit" id="dialog-submit-button">Load Region</wl-button>
            </div>
        </wl-dialog>
        `;
  }
