...
export class ModelView extends connect(store)(PageViewElement) {
  ...
  @property({ type: Object }) private _setup: ModelConfigurationSetup | null =
    null;
  @property({ type: Object }) private _logo!: Image;
  @property({ type: Object }) private _authors: IdMap<Person> =
    {} as IdMap<Person>;
  @property({ type: Object }) private _funding: IdMap<FundingInformation> =
    {} as IdMap<FundingInformation>;
  @property({ type: Object }) private _organizations: IdMap<Organization> =
    {} as IdMap<Organization>;
  @property({ type: Object }) private _timeIntervals: IdMap<TimeInterval> =
    {} as IdMap<TimeInterval>;
  @property({ type: Object }) private _grids: IdMap<Grid> = {} as IdMap<Grid>;
  @property({ type: Object }) private _processes: IdMap<Process> =
    {} as IdMap<Process>;
  @property({ type: Object }) private _images: IdMap<Image> =
    {} as IdMap<Image>;
  @property({ type: Object }) private _visualizations: IdMap<Visualization> =
    {} as IdMap<Visualization>;
  @property({ type: Object }) private _sourceCodes: IdMap<SourceCode> =
    {} as IdMap<SourceCode>;
  @property({ type: Object }) private _softwareImages: IdMap<SoftwareImage> =
    {} as IdMap<SoftwareImage>;
  @property({ type: Object }) private _parameters: IdMap<Parameter> =
    {} as IdMap<Parameter>;
  @property({ type: Object })
  private _datasetSpecifications: IdMap<DatasetSpecification> =
    {} as IdMap<DatasetSpecification>;
  @property({ type: Object })
  private _datasetSpecifications2: IdMap<DatasetSpecification> =
    {} as IdMap<DatasetSpecification>;
  @property({ type: Object }) private _interventions: IdMap<Intervention> =
    {} as IdMap<Intervention>;
  @property({ type: Object })
  ...
  // View controls
  @property({ type: Object }) private _loading: IdMap<boolean> =
    {} as IdMap<boolean>;
  @property({ type: Object }) private _loadedPresentations: IdMap<boolean> =
    {} as IdMap<boolean>;
    ...
    >
      ...
      <div slot="content">
        ...
        <wl-text>
          ...
          (Desktop Application for Model Execution) for documentation and
          installation instructions.
        </wl-text>
      </div>
      <div slot="footer">
        <wl-button
          @click="${() => hideDialog("CLIDialog", this.shadowRoot)}"
  ...
  _updateConfigSelector() {
    let configSelectorWl: Select = this.shadowRoot!.getElementById(
      "config-selector"
    ) as Select;
    let configSelector: HTMLSelectElement | null = configSelectorWl
      ? configSelectorWl.getElementsByTagName("select")[0]
      : null;
    if (configSelectorWl && configSelector) {
      if (this._lastSelectorTimeout != null)
        clearTimeout(this._lastSelectorTimeout);
      ...
      this._model.hasVersion
        .map((ver: SoftwareVersion) =>
          this._versions[ver.id] ? this._versions[ver.id] : ver
        )
        .forEach((ver: SoftwareVersion) => {
          ...
          if (ver.hasConfiguration) {
            ver.hasConfiguration
              .map((cfg: ModelConfiguration) =>
                this._configs[cfg.id] ? this._configs[cfg.id] : cfg
              )
  ...
  _onConfigChange() {
    ...
    if (configSelectorWl && configSelector) {
      ...
      let ver = Object.values(this._versions)
        .filter((ver: SoftwareVersion) =>
          (ver.hasConfiguration || []).some(
            (cfg: ModelConfiguration) => cfg.id === cfgURL
          )
        )
        .pop();
  ...
  _updateSetupSelector() {
    let setupSelectorWl: Select = this.shadowRoot!.getElementById(
      "setup-selector"
    ) as Select;
    let setupSelector: HTMLSelectElement | null = setupSelectorWl
      ? setupSelectorWl.getElementsByTagName("select")[0]
      : null;
    if (setupSelectorWl && setupSelector && this._config) {
      ...
      (this._config.hasSetup || [])
        .map((setup: ModelConfigurationSetup) =>
          this._setups[setup.id] ? this._setups[setup.id] : setup
        )
        .filter((setup: ModelConfigurationSetup) =>
          setupInRegion(setup, this._region.model_catalog_uri, this._regions)
        )
  ...
  _onSetupChange() {
    ...
    if (setupSelectorWl && setupSelector) {
      ...
      goToPage(
        this.PREFIX + getURL(this._model, this._version, this._config, setupURL)
      );
    }
  }
  ...
  _renderSelectors() {
    ...
    return html`
      <div class="config-selector">
        ...
        <span>
          <wl-select
            label=${this._selectedConfig
              ? "Selected configuration"
              : "Select a configuration"}
            id="config-selector"
            @input="${this._onConfigChange}"
          >
          </wl-select>
        </span>
        <span>
          <wl-button
            flat
            inverted
            @click="${() => this._goToEdit(this._selectedConfig)}"
            ?disabled="${!this._selectedConfig || this.user === null}"
          >
          ...
          <span class="tooltip small-tooltip" tip="Download and Run">
            <wl-button
              flat
              inverted
              @click=${() => this._openCLIDialog(this._selectedConfig)}
              ?disabled="${!isExecutable(this._config)}"
            >
              <wl-icon>get_app</wl-icon>
            </wl-button>
          </span>
          <span
            tip="A model configuration is a unique way of running a model, exposing concrete inputs and outputs"
      ...
      <div
        class="setup-selector 
                ${this._selectedConfig &&
        this._config &&
        this._config.hasSetup &&
        this._config.hasSetup.length > 0
          ? ""
          : "hidden"}"
      >
        ${this._selectedSetup
        ...
        <span>
          <wl-select
            label=${this._selectedSetup
              ? "Selected configuration setup"
              : "Select a configuration setup"}
            id="setup-selector"
            @input="${this._onSetupChange}"
          >
          </wl-select>
        </span>
        <span>
          <wl-button
            ...
            inverted
            @click="${() =>
              this._goToEdit(this._selectedConfig, this._selectedSetup)}"
            ?disabled=${!this._selectedSetup || this.user == null}
          >
            <wl-icon>edit</wl-icon>
            <span style="font-size: 11px; font-weight: bold;">
              Edit<br />Setup
          ...
          <span class="tooltip small-tooltip" tip="Download and Run">
            <wl-button
              flat
              inverted
              @click=${() => this._openCLIDialog(this._selectedSetup)}
              ?disabled="${!isExecutable(this._setup)}"
            >
              <wl-icon>get_app</wl-icon>
            </wl-button>
          </span>
          <span
            tip="A model configuration setup represents a model with parameters that have been adjusted (manually or automatically) to be run in a specific region"
            class="tooltip config-tooltip"
            style="top: 4px;"
          >
            <wl-icon style="--icon-size: 24px;">help_outline</wl-icon>
      ...
      <div class="info-center ${hasVersions ? "hidden" : ""}">
        - No version available -
      </div>
      <div
        class="info-center ${this._config &&
        (!this._config.hasSetup || this._config.hasSetup.length === 0)
          ? ""
          : "hidden"}"
      >
        - No configuration setup available
        <a class="clickable" @click="${this._addConfig}">add one</a> -
      </div>
    `;
  }
  ...
  protected render() {
    ...
    if (this._model.dateCreated) {
      creationDateStr = this._model.dateCreated[0]
        .toString()
        .replace(/T.*$/, "");
    ...
    return html` ${this._renderCLIDialog()}
      <div class="col-title">
        <wl-title level="2">
          ...
          <wl-button
            style="--button-font-size: 16px; --button-padding: 8px; float: right;"
            flat
            inverted
          ...
          >
            <wl-icon>edit</wl-icon>
            <span style="font-size: 11px;"> Edit<br />Model </span>
          </wl-button>
        </wl-title>
      </div>
      ...
      ${this._setupNotFound
        ? html` <p style="text-align: center; color:red;">
            The setup
            ${this._selectedSetup
              ? this._selectedSetup.split("/").pop()
              : "selected"}
            is private or was deleted
          </p>`
        : ""}
      ...
      <div class="wrapper">
        <div class="col-img text-centered">
          ...
          ${this._model.hasModelCategory
            ? html`<div>
                <b>Category:</b> ${this._model.hasModelCategory
                  .map(getLabel)
                  .join(", ")}
              </div>`
            : ""}
          ${modelType.length > 0
            ? html`<div><b>Model type:</b> ${modelType.join(", ")}</div>`
            : ""}
        </div>
        <div class="col-desc">
          <wl-text style="text-align: justify;"
            >${this._model.description}</wl-text
          >
          ${this._emulators[this._selectedModel]
          ...
          <div class="desc-ext">
            ${this._model.author
              ? html`<wl-text
                  ><b>• Authors:</b> ${this._renderAuthors(
                    this._model.author
                  )}</wl-text
                >`
              : ""}
            ${this._model.hasFunding
              ? html`<wl-text
                  ><b>• Funding:</b> ${this._renderFundings(
                    this._model.hasFunding
                  )}</wl-text
                >`
              : ""}
            ${this._model.publisher
              ? html`<wl-text
                  ><b>• Publisher:</b>
                  ${this._model.publisher.map(
                    (publisher: Organization) => html`
            ...
            ${this._model.datePublished
              ? html`<wl-text
                  ><b>• Publication date:</b> ${this._model
                    .datePublished}</wl-text
                >`
              : ""}
            ${this._model.citation && this._model.citation.length > 0
              ? html`<wl-text
                  ><b>• Preferred citation:</b>
                  <i>${urlify(this._model.citation[0])}</i></wl-text
                >`
              : ""}
            ${this._model.hasDocumentation
              ...
              : ""}
            ${!this._modelRegions || this._modelRegions.length > 0
              ? html`<wl-text
                  ><b>• Regions:</b>
                  ${!this._modelRegions
                    ? html`<loading-dots style="--width: 20px"></loading-dots>`
                    : this._modelRegions
                        .map((rid: string) => this._regions[rid])
                        .map(
                          (region: Region) =>
                            html`<span class="resource region"
                              >${getLabel(region)}
                            </span>`
                        )}
                </wl-text>`
              : ""}
            ${this._model.hasProcess && this._model.hasProcess.length > 0
              ? html`<wl-text
                  ><b>• Processes:</b>
            ...
            ${this._model.keywords
              ? html`<wl-text
                  ...
                  ${this._model.keywords
                    .map((kws: string) =>
                      kws.split(/ *; */).map(capitalizeFirstLetter).join(", ")
                    )
                    .join(", ")}
                </wl-text>`
              : ""}
        ...
        <div class="row-tab-header">
          <wl-tab-group>
            <wl-tab
              id="tab-overview"
              ?checked=${this._tab == "overview"}
              @click="${() => this._goToTab("overview")}"
              >Overview</wl-tab
            >
            <wl-tab
              id="tab-io"
              ?checked=${this._tab == "io"}
              @click="${() => this._goToTab("io")}"
              >Inputs and Outputs</wl-tab
            >
            <wl-tab
              id="tab-variable"
              ?checked=${this._tab == "variables"}
              @click="${() => this._goToTab("variables")}"
              >Variables</wl-tab
            >
            ${this._getExample()
              ? html` <wl-tab
                  id="tab-example"
                  @click="${() => this._goToTab("example")}"
                >
                  Example
                </wl-tab>`
              : ""}
            <wl-tab
              id="tab-overview"
              ?checked=${this._tab == "tech"}
              @click="${() => this._goToTab("tech")}"
  ...
  private _renderTabTechnical() {
    return html`
      ...
      ${this._model
        ? this._renderTableTechnical(this._model as ModelConfiguration, "MODEL")
        : ""}
      <br />
      ${this._config
        ? this._renderTableTechnical(this._config, "CONFIGURATION")
        : ""}
      <br />
      ${this._selectedSetup && this._loading[this._selectedSetup]
        ? html`
  ...
  private _renderTableTechnical(
    resource: ModelConfiguration | ModelConfigurationSetup,
    titlePrefix: string
  ) {
    //Do no show when the model does not have any technical property
    if (
      !resource.hasSoftwareImage &&
      !resource.operatingSystems &&
      !resource.memoryRequirements &&
      !resource.processorRequirements &&
      !resource.softwareRequirements &&
      !resource.hasDownloadURL &&
      !resource.hasInstallationInstructions &&
      !resource.hasComponentLocation &&
      !resource.hasSourceCode &&
      !resource.parameterization &&
      !resource.runtimeEstimation &&
      !resource.limitations
    ) {
      return html``;
    }

    let instructionsIsUrl: boolean =
      resource.hasInstallationInstructions &&
      resource.hasInstallationInstructions.length > 0 &&
      resource.hasInstallationInstructions[0].substring(0, 4) === "http";

    return html` <table class="pure-table pure-table-striped">
      <thead>
        <tr>
          <th colspan="2">
            ${titlePrefix}:
            <span
              style="margin-left: 6px; font-size: 16px; font-weight: bold; color: black;"
            >
              ${getLabel(resource)}
            </span>
          </th>
      ...
      <tbody>
        ${resource.hasSoftwareImage && resource.hasSoftwareImage.length > 0
          ? html` <tr>
              <td><b>Software image:</b></td>
              <td>
                ${this._loading[resource.hasSoftwareImage[0].id]
                  ? html`${resource.hasSoftwareImage[0].id}
                      <loading-dots style="--width: 20px"></loading-dots>`
                  : html`<a
                      target="_blank"
                      href="https://hub.docker.com/r/${getLabel(
                        this._softwareImages[resource.hasSoftwareImage[0].id]
                      ).split(":")[0]}/tags"
                    >
          ...
          : ""}
        ${resource.operatingSystems && resource.operatingSystems.length > 0
          ? html` <tr>
              <td><b>Operating systems:</b></td>
              <td>${resource.operatingSystems[0].split(";").join(", ")}</td>
            </tr>`
          : ""}
        ${resource.memoryRequirements && resource.memoryRequirements.length > 0
          ? html` <tr>
          ...
          : ""}
        ${resource.processorRequirements &&
        resource.processorRequirements.length > 0
          ? html` <tr>
          ...
          : ""}
        ${resource.softwareRequirements &&
        resource.softwareRequirements.length > 0
          ? html` <tr>
              <td><b>Software requirements:</b></td>
              <td>${resource.softwareRequirements[0]}</td>
            </tr>`
          : ""}
        ${resource.parameterization && resource.parameterization.length > 0
          ? html` <tr>
        ...
        ${resource.limitations && resource.limitations.length > 0
          ...
          : ""}
        ${resource.runtimeEstimation && resource.runtimeEstimation.length > 0
          ? html` <tr>
              <td><b>Runtime Estimation:</b></td>
              <td>${resource.runtimeEstimation[0]}</td>
            </tr>`
          : ""}
        ${resource.hasDownloadURL && resource.hasDownloadURL.length > 0
          ? html` <tr>
              <td><b>Download:</b></td>
              <td>
                <a target="_blank" href="${resource.hasDownloadURL[0]}"
                  >${resource.hasDownloadURL[0]}</a
                >
              </td>
            </tr>`
          : ""}
        ...
        resource.hasInstallationInstructions.length > 0
              ...
              <td>
                ${instructionsIsUrl
                  ? html`
                      <a
                        target="_blank"
                        href="${resource.hasInstallationInstructions[0]}"
                        >${resource.hasInstallationInstructions[0]}</a
                      >
                    `
                  : html`
          ...
          : ""}
        ${resource.hasComponentLocation &&
        resource.hasComponentLocation.length > 0
          ? html` <tr>
              <td><b>Component location:</b></td>
              <td>
                <a target="_blank" href="${resource.hasComponentLocation[0]}"
                  >${resource.hasComponentLocation[0]}</a
                >
              </td>
            </tr>`
          : ""}
        ${resource.hasSourceCode && resource.hasSourceCode.length > 0
          ? html`
              <tr>
                <td><b>Source code:</b></td>
                <td>
                  ${this._loading[resource.hasSourceCode[0].id]
                    ? html`${resource.hasSourceCode[0].id}
                        <loading-dots style="--width: 20px"></loading-dots>`
                    : this._sourceCodes[resource.hasSourceCode[0].id]
                        .codeRepository
                    ? html`<a
                        target="_blank"
                        href="${this._sourceCodes[resource.hasSourceCode[0].id]
                          .codeRepository[0]}"
                      >
                        ${getLabel(
                          this._sourceCodes[resource.hasSourceCode[0].id]
              ...
              this._sourceCodes[resource.hasSourceCode[0].id]
                ...
                : ""}
              ${!this._loading[resource.hasSourceCode[0].id] &&
              this._sourceCodes[resource.hasSourceCode[0].id].license
                ? html` <tr>
                    <td><b>License:</b></td>
                    <td>
                      ${this._sourceCodes[resource.hasSourceCode[0].id].license}
            ...
            `
          : ""}
        ${titlePrefix == "CONFIGURATION" || titlePrefix == "SETUP"
          ? html` <tr>
                ...
                <div class="monospaced">
                  <div>
                    ...
                    ${titlePrefix == "CONFIGURATION"
                      ? uriToId(this._selectedConfig)
                      : uriToId(this._selectedSetup)}
                  </div>
                </div>
              </td>
            </tr>`
  ...
  _changeTab(tabName: string, fragment?: string) {
    ...
    if (ioElement && tabId) {
      ...
      if (fragId) {
        setTimeout(() => {
          let frag: HTMLElement | null =
            this.shadowRoot!.getElementById(fragId);
  ...
  _renderTabOverview() {
    return html` ${this._model.hasPurpose && this._model.hasPurpose.length > 0
      ? html` <wl-title level="2" style="font-size: 16px;"
            >Model purpose:</wl-title
          >
          <ul style="margin-top: 5px">
            ${this._model.hasPurpose.map(
              (p: string) => html`<li>${capitalizeFirstLetter(p)}.</li>`
            )}
          </ul>`
      : ""}
    ${this._model.usefulForCalculatingIndex &&
    this._model.usefulForCalculatingIndex.length > 0
      ? html` <wl-title level="2" style="font-size: 16px;"
            >Relevant for calculating index:</wl-title
          >
          <ul style="margin-top: 5px">
            ${this._model.usefulForCalculatingIndex.map(
              (index: any) => html`
                <li>
                  <a target="_blank" href="${index.id}"> ${getLabel(index)} </a>
                </li>
              `
            )}
          </ul>`
      : ""}
    ${this._model.theoreticalBasis && this._model.theoreticalBasis.length > 0
      ? html` <wl-title
            level="2"
            style="font-size: 16px; display: inline-block; margin-right: 4px;"
            >Theoretical Basis:</wl-title
          >
          <span> ${this._model.theoreticalBasis[0]} </span>`
      : ""}
    ${this._model.hasAssumption && this._model.hasAssumption.length > 0
      ? html` <wl-title level="2" style="font-size: 16px;"
            >Assumptions:</wl-title
          >
          <ul style="margin-top: 5px">
            ${this._model.hasAssumption.map((a: string) =>
              a
                .split(".")
                .filter((txt: string) => !!txt)
                .map((txt: string) => html`<li>${txt}.</li>`)
            )}
          </ul>`
      : ""}
    ${this._config ? this._renderConfigResume() : ""}
    ${this._renderRelatedModels()} ${this._renderGallery()}`;
  }

  _renderConfigResume() {
    return html`
      <fieldset
        style="border-radius: 5px; padding: 0px 7px; border: 2px solid #D9D9D9; margin-bottom: 8px;"
      >
        ...
        <div class="metadata-top-buttons">
          <div class="button-preview" @click=${() => this._changeTab("io")}>
            <div>Parameters</div>
            <div>
              ${this._config.hasParameter
                ? this._config.hasParameter.length
                : 0}
            </div>
          </div>
          <div class="button-preview" @click=${() => this._changeTab("io")}>
            <div>Input files</div>
            <div>
              ${this._config.hasInput ? this._config.hasInput.length : 0}
            </div>
          </div>
          <div class="button-preview" @click=${() => this._changeTab("io")}>
            <div>Output files</div>
            <div>
              ${this._config.hasOutput ? this._config.hasOutput.length : 0}
            </div>
          </div>
        </div>

        <wl-title level="2" style="font-size: 16px;"
          >${this._config.label}</wl-title
        >
        <wl-text>${this._config.description}</wl-text>
        ${this._config.hasUsageNotes && this._config.hasUsageNotes.length > 0
          ? html`
              <br />
              <wl-text>
                <b>Usage notes:</b>
        ...
        <div class="desc-ext" style="padding:10px;">
          ...
          ${this._config.hasFunding && this._config.hasFunding.length > 0
            ? html`<wl-text
                ><b>• Funding Source:</b> ${this._renderFundings(
                  this._config.hasFunding
                )}</wl-text
              >`
            : ""}
          ${this._config.hasRegion && this._config.hasRegion.length > 0
            ? html`<wl-text
                ><b>• Regions:</b> ${this._config.hasRegion.map(
                  (region: Region) =>
                    this._loadingRegions
                      ? html`${getId(region)}
                          <loading-dots style="--width: 20px"></loading-dots>`
                      : html`<span class="resource region"
                          >${getLabel(region)}</span
                        >`
                )}</wl-text
              >`
            : ""}
          ${this._config.hasOutputTimeInterval &&
          this._config.hasOutputTimeInterval.length > 0
            ? html`<wl-text
                style="display: flex; align-items: center; padding: 1px 0px;"
              >
                <b>• Time interval:</b> ${this._renderTimeInterval(
                  this._config.hasOutputTimeInterval[0]
                )}
              </wl-text>`
            : ""}
          ${this._config.hasGrid && this._config.hasGrid.length > 0
            ? html`<wl-text
                style="display: flex; align-items: center; padding: 1px 0px;"
              >
                <b>• Grid:</b> ${this._renderGrid(
                  this._config.hasGrid[0]
                )}</wl-text
              >`
            : ""}
          ${this._config.hasProcess && this._config.hasProcess.length > 0
            ? html`<wl-text
                ><b>• Processes:</b> ${this._renderProcesses(
  ...
  _renderSetupResume() {
    ...
    let paramLen: number = this._setup.hasParameter
      ? this._setup.hasParameter.length
      : 0;
    let inputLen: number = this._setup.hasInput
      ? this._setup.hasInput.length
      : 0;
    let paramFixed: number = (this._setup.hasParameter || []).filter(
      (p: Parameter) => !!p.hasFixedValue
    ).length;
    let inputFixed: number = (this._setup.hasInput || []).filter(
      (i: DatasetSpecification) => !!i.hasFixedResource
    ).length;
    let configProcesses: string[] = (this._config.hasProcess || []).map(
      (p: Process) => p.id
    );
    let setupProcesses: Process[] = (this._setup.hasProcess || []).filter(
      (p: Process) => !configProcesses.includes(p.id)
    );
    return html`
      <fieldset
        style="border-radius: 5px; padding: 0px 7px; border: 2px solid #D9D9D9; margin-bottom: 8px;"
      >
        ...
        <wl-title level="2" style="font-size: 16px;"
          >${this._setup.label}</wl-title
        >
        <wl-text>${this._setup.description}</wl-text>
        ${this._setup.hasUsageNotes && this._setup.hasUsageNotes.length > 0
          ? html`
              <br />
              <wl-text>
                <b>Usage notes:</b>
        ...
        <div class="desc-ext" style="padding:10px;">
          ...
          this._setup.parameterAssignmentMethod.length > 0
            ? html`<wl-text>
                <b>• Parameter assignment method:</b> ${this._setup
                  .parameterAssignmentMethod[0]}
              </wl-text>`
            : ""}
          ...
          ${this._setup.hasFunding && this._setup.hasFunding.length > 0
            ? html`<wl-text
                ><b>• Funding Source:</b> ${this._renderFundings(
                  this._setup.hasFunding
                )}</wl-text
              >`
            : ""}
          ${this._setup.hasRegion && this._setup.hasRegion.length > 0
            ? html`<wl-text
                ><b>• Regions:</b> ${this._setup.hasRegion.map(
                  (region: Region) =>
                    this._loadingRegions
                      ? html`${getId(region)}
                          <loading-dots style="--width: 20px"></loading-dots>`
          ...
          (!this._config.hasOutputTimeInterval ||
            this._config.hasOutputTimeInterval.length == 0 ||
            this._config.hasOutputTimeInterval[0].id !=
              this._setup.hasOutputTimeInterval[0].id)
            ? html`<wl-text display: flex; align-items: center; padding: 1px 0px;>
                            <b>• Time interval:</b> ${this._renderTimeInterval(
                              this._setup.hasOutputTimeInterval[0]
                            )}
                        </wl-text>`
            : ""}
          ${this._setup.hasGrid &&
          this._setup.hasGrid.length > 0 &&
          (!this._config.hasGrid ||
            this._config.hasGrid.length === 0 ||
            this._config.hasGrid[0].id != this._setup.hasGrid[0].id)
            ? html`<wl-text
                style="display: flex; align-items: center; padding: 1px 0px;"
              >
                <b>• Grid:</b> ${this._renderGrid(
                  this._setup.hasGrid[0]
                )}</wl-text
              >`
            : ""}
          ${setupProcesses.length > 0
            ? html`<wl-text
              ...
              >`
            : ""}
          ${this._setup.adjustableParameter &&
          this._setup.adjustableParameter.length > 0
            ? html`<wl-text
                ><b>• Adjustable parameters:</b>
                ${this._setup.adjustableParameter.map(
                  (p: Parameter, i: number) =>
                    i === 0
                      ? html`<code
                          class="clickable"
                          @click="${() => this._changeTab("io", "parameters")}"
                        >
                          ${getLabel(p)}
                        </code>`
                      : html`,
              ...
              >`
            : ""}
          ${this._setup.calibrationTargetVariable &&
          this._setup.calibrationTargetVariable.length > 0
            ? html`<wl-text
                ><b>• Target variables:</b>
                ${this._setup.calibrationTargetVariable.map(
                  (v: VariablePresentation, i: number) =>
                    i === 0
    ...
    if (!this._config) {
      return html` <br />
        <h3 style="margin-left:30px">
          You must select a configuration or setup to see its files and
          parameters.
        </h3>`;
    }
    return html`
      ...
      <wl-text style="font-style: italic; padding-left: 20px; margin-top: 1em;">
        Look at the Variables tab to see more information about the contents of
        the inputs and outputs.
      </wl-text>
    `;
  }

  private _renderInputFileTable(
    resource: ModelConfiguration | ModelConfigurationSetup
  ) {
          ...
          ${isSetup
            ? html` <th style="text-align: right;">
                ...
                <span
                  ...
                  tip="If a value is not set up in this field, the configuration default value will be used."
                >
      ...
      <tbody>
        ${!resource.hasInput || resource.hasInput.length === 0
          ? html` <tr>
              <td colspan="4">
                <div class="text-centered">
                  This ${isSetup ? "setup" : "configuration"} has no input
                  files.
                </div>
              </td>
            </tr>`
          : html` ${resource.hasInput
              .filter((ds: DatasetSpecification) => !this._loading[ds.id])
              .map(
                (ds: DatasetSpecification) => this._datasetSpecifications[ds.id]
              )
              .sort(sortByPosition)
              .map(
                (ds: DatasetSpecification) => html`
                  <tr>
                    <td><span class="monospaced">${getLabel(ds)}</span></td>
                    <td>
                      ${ds.description && ds.description.length > 0
                        ? ds.description
                        : ""}
                    </td>
                    ${isSetup
                    ...
                    <td style="text-align: right;" class="number">
                      ${ds.hasFormat && ds.hasFormat.length > 0
                        ? ds.hasFormat[0]
                        : ""}
                    </td>
                  </tr>
  ...
  _renderParametersTable(
    ...
    >
          ...
          <th style="text-align: right;">
            ${isSetup
              ? html` Value on setup
                  <span
                    class="tooltip table-tooltip"
                    tip="If a value is not set up in this field configuration default value will be used."
                  >
                    <wl-icon>help</wl-icon>
                  </span>`
              : "Default value"}
      ...
      <tbody>
        ${!resource.hasParameter || resource.hasParameter.length === 0
          ? html` <tr>
          ...
          : html` ${resource.hasParameter
              .filter((p: Parameter) => !this._loading[p.id])
              .map((p: Parameter) => this._parameters[p.id])
              .sort(sortByPosition)
              .map(
                (p: Parameter) => html`
                  <tr>
                    <td>
                      ...
                      ${p.hasMinimumAcceptedValue &&
                      p.hasMinimumAcceptedValue.length > 0
                        ? html`
                            <br />The range is from
                            ${p.hasMinimumAcceptedValue[0]}
                            ${p.hasMaximumAcceptedValue &&
                            p.hasMaximumAcceptedValue.length > 0
                      ...
                      ${p.description && p.description.length > 0
                        ? html`
                            <b style="font-size: 14px;"
                              >${capitalizeFirstLetter(p.description[0])}</b
                            >
                          `
                        : ""}
                    </td>
                    <td>
                      ${p.relevantForIntervention &&
                      p.relevantForIntervention.length > 0 &&
                      p.relevantForIntervention[0].id
                        ? html`
                            <span
                    ...
                    <td class="font-numbers" style="text-align: right;">
                      ${isSetup
                        ...
                        : p.hasDefaultValue}
                      ${p.usesUnit && p.usesUnit.length > 0
                        ? getLabel(p.usesUnit[0])
                        : ""}
                    </td>
                  </tr>
                `
              )}
            ${resource.hasParameter
              .filter((p: Parameter) => this._loading[p.id])
              .map(
                (p: Parameter) => html`
                  <tr>
  ...
  private _renderOutputFileTable(
    resource: ModelConfiguration | ModelConfigurationSetup
  ) {
    let isSetup = resource.type.includes("ModelConfigurationSetup");
    return html` <table
    ...
    >
      ...
      <tbody>
        ${!resource.hasOutput || resource.hasOutput.length === 0
          ? html` <tr>
              <td colspan="4">
                <div class="text-centered">
                  This ${isSetup ? "setup" : "configuration"} has no output
                  files.
                </div>
              </td>
            </tr>`
          : html` ${resource.hasOutput
              .filter((ds: DatasetSpecification) => !this._loading[ds.id])
              .map(
                (ds: DatasetSpecification) => this._datasetSpecifications[ds.id]
              )
              .sort(sortByPosition)
              .map(
                (ds: DatasetSpecification) => html` <tr>
                  <td><span class="monospaced">${getLabel(ds)}</span></td>
                  <td colspan="${isSetup ? 2 : 1}">
                    ${ds.description && ds.description.length > 0
                      ? ds.description
                      : ""}
                  </td>
  ...
  private _renderFixedResource(ds: DatasetSpecification) {
    ...
    return html`
      ${ds.hasFixedResource &&
      ds.hasFixedResource.length > 0 &&
      ds.hasFixedResource[0].value &&
      ds.hasFixedResource[0].value.length > 0
        ? html`
            <a target="_blank" href="${ds.hasFixedResource[0].value[0]}">
              ${((<unknown>ds.hasFixedResource[0].value[0]) as string)
                .split("/")
                .pop()}
  ...
  private _renderTabVariables() {
    let resource: ModelConfiguration | ModelConfigurationSetup = this._setup
      ? this._setup
      : this._config;
    return html`
      ${this._config
        ? html`
            <wl-title level="3"
              >${this._setup ? "Setup" : "Configuration"} variables:</wl-title
            >
            ...
            (!resource.hasOutput || resource.hasOutput.length === 0)
              ? html` <br />
                  <h3 style="margin-left:30px">
                    This information has not been specified in the
                    ${this._setup ? "setup" : "configuration"} yet.
                  </h3>`
              : ""}
          `
        : ""}
      ${this._model &&
      (this._model.hasInputVariable || this._model.hasOutputVariable)
        ? html`
            <wl-title level="3" style="margin-top: 1em;">
              Model variables:</wl-title
            >
            ${this._model.hasInputVariable &&
            this._model.hasInputVariable.length > 0
              ? html`
                  <wl-title level="2" style="font-size: 16px;"
                    >Input Variables:</wl-title
                  >
                  ${this._renderVariablePresentationTable(
                    this._model.hasInputVariable
                  )}
                `
              : ""}
            ${this._model.hasOutputVariable &&
            this._model.hasOutputVariable.length > 0
              ? html`
                  <wl-title level="2" style="font-size: 16px;"
                    >Output Variables:</wl-title
                  >
                  ${this._renderVariablePresentationTable(
                    this._model.hasOutputVariable
                  )}
  ...
  private _renderExpansionVariables(
    dsArr: DatasetSpecification[],
    title: string
  ) {
    ...
    return html`
      ...
      ${dsArr.map(
        (ds: DatasetSpecification) => html` <wl-expansion
          id="${getLabel(ds)}"
          name="${title}"
          @click="${() => this._expandDS(ds)}"
        ...
        >
          ...
          ${this._loading[ds.id]
            ...
            : html`
                <div
                  style="font-style: oblique; margin-bottom: 1em; text-align: justify; font-size: 0.9em;"
                >
                  ${this._datasetSpecifications[ds.id].description}
                </div>
              `}
          ${this._loading[ds.id] ||
          (!this._loading[ds.id] && !this._loadedPresentations[ds.id])
            ? html`<div class="text-centered">
                <wl-progress-spinner></wl-progress-spinner>
              </div>`
            : !this._datasetSpecifications[ds.id].hasPresentation ||
              this._datasetSpecifications[ds.id].hasPresentation.length === 0
            ? html` <div class="text-centered">
                <h4>
                  There is no description available for the variables in this
                  file.
                </h4>
              </div>`
            : html`
                <table class="pure-table pure-table-bordered">
                  ...
                  <tbody>
                    ${this._datasetSpecifications[ds.id].hasPresentation
                      .filter(
                        (vp: VariablePresentation) => !this._loading[vp.id]
                      )
                      .map(
                        (vp: VariablePresentation) =>
                          this._variablePresentations[vp.id]
                      )
                      .map(
                        (vp: VariablePresentation) => html`
                          <tr>
                            <td>${getLabel(vp)}</td>
                            ...
                            <td style="min-width: 80px;">
                              ${vp.usesUnit && vp.usesUnit.length > 0
                                ? this._units[vp.usesUnit[0].id]
                                  ? getLabel(this._units[vp.usesUnit[0].id])
                                  : html`<loading-dots
                                      style="--height: 10px; margin-left:10px"
                                    ></loading-dots>`
                                : "-"}
  ...
  private _renderVariablePresentationTable(vps: VariablePresentation[]) {
    return html`
      <table class="pure-table pure-table-bordered">
        ...
        <tbody>
          ${vps.map((vp: VariablePresentation) =>
            this._loading[vp.id]
              ? html`
                  <tr>
                    <td colspan="5">
              ...
              : html`
                  <tr>
                    ...
                    <td style="word-wrap: break-word;">
                      ${this._variablePresentations[vp.id]
                        .hasStandardVariable &&
                      this._variablePresentations[vp.id].hasStandardVariable
                        .length > 0
                        ? html`
                            <a
                              class="monospaced link"
                              target="_blank"
                              href="${this._variablePresentations[vp.id]
                                .hasStandardVariable[0].id}"
                            >
                              ${getLabel(
                                this._variablePresentations[vp.id]
                                  .hasStandardVariable[0]
                              )}
                            </a>
                          `
                        : "-"}
                    ...
                    <td style="min-width: 80px;">
                      ...
                      this._variablePresentations[vp.id].usesUnit.length > 0
                        ? getLabel(
                            this._variablePresentations[vp.id].usesUnit[0]
                          )
                        : "-"}
                    </td>
                  </tr>
  ...
  _renderGallery() {
    ...
    (this._model.hasExplanationDiagram || []).forEach((image: Image) =>
      allImages.add(image.id)
    );
    (this._model.screenshot || []).forEach((image: Image) =>
      allImages.add(image.id)
    );
    (this._model.hasSampleVisualization || []).forEach((viz: Visualization) =>
      allVisualizations.add(viz.id)
    );

    if (this._version) {
      (this._version.screenshot || []).forEach((image: Image) =>
        allImages.add(image.id)
      );
      (this._version.hasSampleVisualization || []).forEach(
        (viz: Visualization) => allVisualizations.add(viz.id)
      );
    }

    if (this._config) {
      (this._config.screenshot || []).forEach((image: Image) =>
        allImages.add(image.id)
      );
      (this._config.hasSampleVisualization || []).forEach(
        (viz: Visualization) => allVisualizations.add(viz.id)
      );
    }
    ...
    let allRes: (Image | Visualization)[] = Array.from(allImages)
      ...
      .filter(
        (img: Image) =>
          img.label && img.label.length > 0 && img.value && img.value.length > 0
      )
      .concat(
    ...
    let stillLoading: boolean =
      allImages.size + allVisualizations.size != allRes.length;
    ...
    let items: GalleryEntry[] = allRes.map((res: Image | Visualization) => {
      ...
      if (
        res.hadPrimarySource &&
        res.hadPrimarySource.length > 0 &&
        res.hadPrimarySource[0]["id"]
      )
        item.source = {
    ...
    if (items.length > 0) {
      return html` ${stillLoading
          ? html` <div style="float: right; margin: 1em 0;">
  ...
  updated() {
    ...
    if (this._tab == "example") {
      ...
      (this.shadowRoot.querySelectorAll("wl-expansion") || []).forEach(
        (wle: any) => {
  ...
  private _getExample() {
    if (
      this._setup &&
      this._setup.hasExample &&
      this._setup.hasExample.length > 0
    )
      return this._setup.hasExample[0];
    if (
      this._config &&
      this._config.hasExample &&
      this._config.hasExample.length > 0
    )
      return this._config.hasExample[0];
    if (
      this._version &&
      this._version.hasExample &&
      this._version.hasExample.length > 0
    )
      return this._version.hasExample[0];
    if (
      this._model &&
      this._model.hasExample &&
      this._model.hasExample.length > 0
    )
  ...
  stateChanged(state: RootState) {
    ...
    let versionChanged: boolean =
      ui && (modelChanged || ui.selectedVersion !== this._selectedVersion);
    let configChanged: boolean =
      ui && (versionChanged || ui.selectedConfig !== this._selectedConfig);
    let setupChanged: boolean =
      ui && ui.selectedCalibration !== this._selectedSetup;
    if (db) {
      ...
      if (modelChanged) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.model.get(this._selectedModel))
          .then((model: Model) => {
            ...
            if (this._model.logo && this._model.logo.length > 0) {
              ...
              if (db.image[logoId]) {
                ...
                store
                  .dispatch(ModelCatalogApi.myCatalog.image.get(logoId))
                  .then((logo: Image) => {
            ...
            this._loadImages(
              (this._model.screenshot || []).concat(
                this._model.hasExplanationDiagram || []
              ),
              db
            );
                ...
                .forEach((ver: SoftwareVersion) => {
                  (ver.hasConfiguration || [])
                    ...
                    .forEach((cfg: ModelConfiguration) => {
                      ...
                      (cfg.hasSetup || [])
                        .map(
                          (setup: ModelConfigurationSetup) =>
                            this._setups[setup.id]
                        )
                        .forEach((setup: ModelConfigurationSetup) => {
      ...
      if (configChanged) {
        ...
        if (ui.selectedConfig) {
          if (db.modelconfiguration[ui.selectedConfig]) {
            ...
            this._loadImages(
              (this._config.screenshot || []).concat(
                this._config.hasExplanationDiagram || []
              ),
              db
            );
      ...
      if (setupChanged) {
        ...
        if (this._selectedSetup) {
          let detailsRequest: Promise<ModelConfigurationSetup> = store.dispatch(
            ModelCatalogApi.myCatalog.modelConfigurationSetup.getDetails(
              this._selectedSetup
            )
          );
          detailsRequest.then((setup: ModelConfigurationSetup) => {
            ...
            if (setup.hasParameter)
              this._loadParameters(setup.hasParameter, db);
            if (setup.hasInput)
              this._loadDatasetSpecifications(setup.hasInput, db);
            if (setup.hasOutput)
              this._loadDatasetSpecifications(setup.hasOutput, db);

            (setup.hasInput || [])
              .concat(setup.hasOutput || [])
              .forEach((ds: DatasetSpecification) => {
          ...
          .forEach((ver: SoftwareVersion) => {
            (ver.hasConfiguration || [])
              ...
              .forEach((cfg: ModelConfiguration) => {
                ...
                (cfg.hasSetup || [])
                  .map(
                    (setup: ModelConfigurationSetup) => this._setups[setup.id]
                  )
                  .forEach((setup: ModelConfigurationSetup) => {
                    (setup && setup.hasRegion ? setup.hasRegion : [])
                      .map((region: Region) => db.region[region.id])
  ...
  private _loadAuthors(
    authorArr: (Person | Organization)[],
    db: ModelCatalogState
  ) {
    (authorArr || []).forEach((author: Person | Organization) => {
      if (author.type && author.type.includes("Person")) {
        if (db.person[author.id]) {
          ...
          store
            .dispatch(ModelCatalogApi.myCatalog.person.get(author.id))
            .then((person: Person) => {
        ...
        if (db.organization[author.id]) {
          ...
          this._loading[author.id] = true;
          store
            .dispatch(ModelCatalogApi.myCatalog.organization.get(author.id))
  ...
  private _renderAuthors(authorArray: (Person | Organization)[]) {
    ...
    return (authorArray || []).map((author: Person | Organization) => {
      if (this._loading[author.id]) {
        return html`${getId(author)}
          <loading-dots style="--width: 20px"></loading-dots>&nbsp;`;
      } else {
        if (author.type && author.type.includes("Person")) {
          return html`<span class="resource author"
            >${getLabel(this._authors[author.id])}</span
          >`;
        } else if (author.type && author.type.includes("Organization")) {
          return html`<span class="resource organization"
            >${getLabel(this._organizations[author.id])}</span
          >`;
        } else {
  ...
  private _loadFundings(
    fundArray: FundingInformation[],
    db: ModelCatalogState
  ) {
      ...
      Promise.all(
        fundArray.map((fund: FundingInformation) => {
          if (db.fundinginformation[fund.id]) {
            ...
            (this._funding[fund.id].fundingSource || []).forEach(
              (org: Organization) => {
            ...
            let req = store.dispatch(
              ModelCatalogApi.myCatalog.fundingInformation.get(fund.id)
            );
        ...
        fundings.forEach((fund: FundingInformation) => {
          (fund.fundingSource || []).forEach((org: Organization) => {
            if (db.organization[org.id]) {
              ...
              store
                .dispatch(ModelCatalogApi.myCatalog.organization.get(org.id))
                .then((organization: Organization) => {
  ...
  private _renderFundings(fundArray: FundingInformation[]) {
    return (fundArray || []).map((fund: FundingInformation) =>
      this._loading[fund.id]
        ? html`${getId(fund)}
            <loading-dots style="--width: 20px"></loading-dots>&nbsp;`
  ...
  private _loadTimeIntervals(tiArr: TimeInterval[], db: ModelCatalogState) {
    (tiArr || []).forEach((ti: TimeInterval) => {
      if (db.timeinterval[ti.id]) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.timeInterval.get(ti.id))
          .then((ti: TimeInterval) => {
    ...
    return this._loading[ti.id]
      ...
      : html`<span class="resource time-interval">
          <span>
            ...
            ${this._timeIntervals[ti.id].intervalUnit &&
            this._timeIntervals[ti.id].intervalUnit.length > 0
  ...
  private _loadGrids(gridArr: Grid[], db: ModelCatalogState) {
    (gridArr || []).forEach((grid: Grid) => {
      if (db.grid[grid.id]) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.grid.get(grid.id))
          .then((grid: Grid) => {
      ...
      : html`<span class="resource grid">
          <div style="display: flex; justify-content: space-between;">
            ...
            <span style="margin-left:20px;">
              ${this._grids[grid.id].hasDimension &&
              this._grids[grid.id].hasDimension.length > 0
                ? html` <span>Dimensions:</span>
                    <span class="number">
          ...
          <div style="display: flex; justify-content: space-between;">
            ${this._grids[grid.id].hasSpatialResolution &&
            this._grids[grid.id].hasSpatialResolution.length > 0
              ? html` <span>
              ...
              : ""}
            ${this._grids[grid.id].hasShape &&
            this._grids[grid.id].hasShape.length > 0
              ? html` <span
                  style="${this._grids[grid.id].hasSpatialResolution &&
                  this._grids[grid.id].hasSpatialResolution.length > 0
                    ? "margin-left:20px;"
                    : ""}"
                >
  ...
  private _loadProcesses(processArr: Process[], db: ModelCatalogState) {
    (processArr || []).forEach((process: Process) => {
      if (db.process[process.id]) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.process.get(process.id))
  ...
  private _loadImages(imagesArr: Image[], db: ModelCatalogState) {
    (imagesArr || []).forEach((image: Image) => {
      if (db.image[image.id]) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.image.get(image.id))
          .then((image: Image) => {
  ...
  private _loadSourceCodes(sourceArr: SourceCode[], db: ModelCatalogState) {
    (sourceArr || []).forEach((sourceCode: SourceCode) => {
      if (db.sourcecode[sourceCode.id]) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.sourceCode.get(sourceCode.id))
  ...
  private _loadSoftwareImages(siArr: SoftwareImage[], db: ModelCatalogState) {
    (siArr || []).forEach((softwareImage: SoftwareImage) => {
      if (db.softwareimage[softwareImage.id]) {
        this._softwareImages[softwareImage.id] =
          db.softwareimage[softwareImage.id];
      } else if (!this._loading[softwareImage.id]) {
        this._loading[softwareImage.id] = true;
        store
          .dispatch(
            ModelCatalogApi.myCatalog.softwareImage.get(softwareImage.id)
          )
          .then((softwareImage: SoftwareImage) => {
      ...
      if (db.parameter[parameter.id]) {
        ...
        if (this._parameters[parameter.id].relevantForIntervention)
          this._loadInterventions(
            this._parameters[parameter.id].relevantForIntervention,
            db
          );
      } else if (!this._loading[parameter.id]) {
        this._loading[parameter.id] = true;
        store
          .dispatch(ModelCatalogApi.myCatalog.parameter.get(parameter.id))
  ...
  private _loadDatasetSpecifications(
    dsArr: DatasetSpecification[],
    db: ModelCatalogState
  ) {
    (dsArr || []).forEach((ds: DatasetSpecification) => {
      if (db.datasetspecification[ds.id]) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.datasetSpecification.get(ds.id))
          .then((datasetSpecification: DatasetSpecification) => {
            this._loading[datasetSpecification.id] = false;
            this._datasetSpecifications[datasetSpecification.id] =
              datasetSpecification;
            this.requestUpdate();
          });
      }
  ...
  private _loadInterventions(
    interventionsArr: Intervention[],
    db: ModelCatalogState
  ) {
    (interventionsArr || []).forEach((intervention: Intervention) => {
      if (db.intervention[intervention.id]) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.intervention.get(intervention.id))
  ...
  private _loadVariablePresentations(
    vpArr: VariablePresentation[],
    db: ModelCatalogState
  ) {
    (vpArr || []).forEach((vp: VariablePresentation) => {
      if (db.variablepresentation[vp.id]) {
        ...
        store
          .dispatch(ModelCatalogApi.myCatalog.variablePresentation.get(vp.id))
          .then((variablePresentation: VariablePresentation) => {
            this._loading[variablePresentation.id] = false;
            this._variablePresentations[variablePresentation.id] =
              variablePresentation;
            this.requestUpdate();
          });
      }
