...
export class ModelsCompare extends connect(store)(PageViewElement) {
  ...
  private _gets = {
    ...
    ModelConfigurationSetup:
      ModelCatalogApi.myCatalog.modelConfigurationSetup.get,
  };
  ...
  private _comparisonFeatures: Array<ComparisonFeature> = [
    {
      ...
      fn: (m: modelLike) =>
        m.type && m.type.length > 0
          ? getModelTypeNames(m.type).join(", ")
          : html`<span style="color:#999">None</span>`,
    },
    {
      name: "Category",
      fn: (setup: ModelConfigurationSetup) =>
    ...
    },
    {
      ...
      fn: (model: ModelConfigurationSetup) => {
        ...
        else return html`<span style="color:#999">None specified</span>`;
      },
    },
    {
      name: "Authors",
    ...
    },
    {
      name: "Description",
      fn: (setup: ModelConfigurationSetup) =>
        setup.description && setup.description.length > 0
          ? setup.description[setup.description.length - 1]
          : html`<span style="color:#999">None provided</span>`,
    },
    {
      name: "Theoretical Basis",
      fn: (model: ModelConfigurationSetup) =>
        model.theoreticalBasis && model.theoreticalBasis.length > 0
          ? model.theoreticalBasis[0]
          : html`<span style="color:#999">None</span>`,
    },
    {
      name: "Runtime Estimation",
      fn: (model: ModelConfigurationSetup) =>
        model.runtimeEstimation && model.runtimeEstimation.length > 0
          ? model.runtimeEstimation[0]
          : html`<span style="color:#999">None</span>`,
    },
    {
      name: "Parameterization",
      fn: (model: ModelConfigurationSetup) =>
        model.parameterization && model.parameterization.length > 0
    ...
    },
    {
      ...
      fn: (m: ModelConfiguration | ModelConfigurationSetup) =>
        m.hasProcess && m.hasProcess.length > 0
          ? this._iProcess[m.id]
          : html`<span style="color:#999">None specified</span>`,
    },

    {
      ...
      fn: (m: ModelConfiguration | ModelConfigurationSetup) =>
        m.hasInputVariable && m.hasInputVariable.length > 0
    ...
    },
    {
      ...
      fn: (m: ModelConfiguration | ModelConfigurationSetup) =>
        m.hasOutputVariable && m.hasOutputVariable.length > 0
...

    {
      name: "Regions",
      fn: (m: ModelConfiguration | ModelConfigurationSetup) =>
        m.hasRegion && m.hasRegion.length > 0
          ? this._iRegion[m.id]
          : html`<span style="color:#999">None specified</span>`,
    },
    {
      ...
      fn: (model: ModelConfigurationSetup) =>
        model.parameterAssignmentMethod &&
        model.parameterAssignmentMethod.length > 0
          ? model.parameterAssignmentMethod[
              model.parameterAssignmentMethod.length - 1
            ]
          : html`<span style="color:#999">None</span>`,
    },

    {
      ...
      fn: (setup: ModelConfigurationSetup) =>
        setup.adjustableParameter && setup.adjustableParameter.length > 0
          ? setup.adjustableParameter.map(
              (p) => html`<span class="resource">${getLabel(p)}</span>`
            )
          : html`<span style="color:#999">None specified</span>`,
    },
    {
      name: "Inputs",
      fn: (m: ModelConfiguration | ModelConfigurationSetup) =>
        m.hasInput && m.hasInput.length > 0
          ? this._iInput[m.id]
          : html`<span style="color:#999">None specified</span>`,
    },
    {
      name: "Grid",
      fn: (m: Model | ModelConfiguration | ModelConfigurationSetup) =>
        m.hasGrid && m.hasGrid.length > 0
          ? this._iGrid[m.id]
          : html`<span style="color:#999">None specified</span>`,
    },
    {
      name: "Time Interval",
      fn: (m: ModelConfiguration | ModelConfigurationSetup) =>
        m.hasOutputTimeInterval && m.hasOutputTimeInterval.length > 0
    ...
    },
    {
      name: "Parameters",
      fn: (m: ModelConfiguration | ModelConfigurationSetup) =>
        m.hasParameter && m.hasParameter.length > 0
    ...
    },
    {
      ...
      fn: (setup: ModelConfigurationSetup) =>
        setup.hasComponentLocation && setup.hasComponentLocation.length > 0
          ? html`<span style="word-break: break-all;"
              >${setup.hasComponentLocation[
                setup.hasComponentLocation.length - 1
              ]}</span
            >`
          : html`<span style="color:#999">None specified</span>`,
    },
    {
      name: "Software Image",
      fn: (m: ModelConfiguration | ModelConfigurationSetup) =>
        m.hasSoftwareImage && m.hasSoftwareImage.length > 0
    ...
    return html`
      <div class="twocolumns">
        <div class="${this._hideLateral ? "left_closed" : "left"}">
          <div class="clt">
            <wl-title level="4" style="margin: 4px; padding: 10px;"
              >Select models:</wl-title
            >
        ...
        <div class="${this._hideLateral ? "right_full" : "right"}">
          <div class="card2">
            ...
            ${this._compare.length > 0
              ? this._renderTable()
              : html` <wl-title level="4"
                  >Select a model on the left panel.</wl-title
                >`}
          </div>
        </div>
      </div>
  ...
  private _renderTable() {
    ...
    return html`
      <table
        class="pure-table pure-table-striped ${bigTable
          ? "horizontal-table"
          : ""}"
      >
        <thead>
          ...
          ${this._compare.map((c: ComparisonEntry) => {
            return html` <th
              .style="width:${bigTable
                ? "380px"
                : (100 / this._compare.length).toString() + "%"}"
            >
              ${this._loading[c.uri]
                ? html`${uriToId(c.uri)}
                    <loading-dots
                ...
                : html`
                    ...
                    <wl-title level="4"
                      >${getLabel(this._dbs[c.type][c.uri])}</wl-title
                    >
                  `}
            </th>`;
          })}
  ...
  _removeFromComparison(c: ComparisonEntry) {
    let newC: ComparisonEntry[] = this._compare.filter(
      (ce: ComparisonEntry) => ce.uri !== c.uri
    );
      ...
      this._compare.forEach((c: ComparisonEntry) => {
        if (!this._loading[c.uri] && !this._dbs[c.type][c.uri]) {
          ...
          store.dispatch(get(c.uri)).then((r: modelLike) => {
            ...
            if (
              r["hasOutputTimeInterval"] &&
              r["hasOutputTimeInterval"].length > 0
            ) {
              this._iTimeInterval[c.uri] = new ModelCatalogTimeInterval();
              this._iTimeInterval[c.uri].setResources(
                r["hasOutputTimeInterval"]
              );
            }
