...
export class RegionDatasets extends connect(store)(RegionQueryPage)  {
    ...
    _isGPM (ds: Dataset) {
        return (ds.name.toLowerCase() + ds.description.toLowerCase()).includes('gpm') ||
    ...
    protected render() {
        ...
        return html`
            ${this._selectedRegion ? 
                html`
                <wl-title level="4" style="font-size: 17px;">Datasets with resources in ${this._selectedRegion.name}</wl-title>
                ${(!this._datasets || this._datasets.loading) ? 
                    ...
                    this._datasets.datasets.map((ds) => html`
                    <wl-list-item class="active" @click="${() => goToPage('datasets/browse/'+ds.id+data_link_suffix)}">
                        <wl-icon slot="before">folder</wl-icon>
                        ${this._isGPM(ds)? html`
                        <a slot="after" target="_blank" href="${this._transform(ds)}">
                            <wl-button @click="${(ev:any) => ev.stopPropagation()}">Transform</wl-button>
                        </a>
                        `:''}
