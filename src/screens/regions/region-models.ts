...
import {
  queryDatasetResourcesAndSave,
  queryDatasetResourcesRaw,
...
export class RegionModels extends connect(store)(PageViewElement) {
  ...
  @property({ type: Array })
  private _matchingSetups: ModelConfigurationSetup[] = [];
  @property({ type: Object }) private _categorizedMatchingSetups: IdMap<
    ModelConfigurationSetup[]
  > = {};
  ...
  _getModelURL(setupURI: string) {
    ...
    let configs = Object.values(this._configs).filter(
      (c: any) =>
        c.hasSetup &&
        c.hasSetup.filter((s: any) => s.id === setupURI).length > 0
    );
    ...
    let versions = Object.values(this._versions).filter(
      (v: any) =>
        v.hasConfiguration &&
        v.hasConfiguration.filter((c: any) => c.id === config.id).length > 0
    );
    ...
    let models = Object.values(this._models).filter(
      (m: any) =>
        m.hasVersion &&
        m.hasVersion.filter((v: any) => v.id === version.id).length > 0
    );
  ...
  private _getMatchingModels() {
    ...
    let selArea: number =
      (selbox.xmax - selbox.xmin) * (selbox.ymax - selbox.ymin);
        ...
        (region.geo || []).forEach((geo: GeoShape) => {
          ...
          if (geoshape && bbox) {
            if (
              bbox &&
              bbox.xmin &&
              doBoxesIntersect(bbox, selbox) &&
              isSubregion(parentRegion, region)
            ) {
    ...
    Object.values(this._setups).forEach((setup: ModelConfigurationSetup) => {
      if (
        (setup.hasRegion || []).some((region: Region) => regions.has(region.id))
      ) {
        setups.add(setup.id);
      }
    });
    ...
    this._matchingSetups = Array.from(setups).map(
      (sid: string) => this._setups[sid]
    );

    this._categorizedMatchingSetups = this._matchingSetups.reduce(
      (
        map: IdMap<ModelConfigurationSetup[]>,
    ...
    Promise.all(
      this._matchingSetups.map((setup: ModelConfigurationSetup) =>
        store.dispatch(
          ModelCatalogApi.myCatalog.modelConfigurationSetup.getDetails(setup.id)
        )
      )
    )
      .then((setups: ModelConfigurationSetup[]) => {
        ...
        if (datasets.size > 0) {
          Promise.all(
            Array.from(datasets).map((ds: string) =>
              queryDatasetResourcesRaw(
                ds,
  ...
  private _setSetupPreview(setup: ModelConfigurationSetup) {
    ...
    store.dispatch(
      setPreview(
        Array.from(selGeo).map((gid) =>
          getBoundingBoxFromGeoShape(this._geoShapes[gid])
        )
      )
    );
  }
  ...
  protected render() {
    if (!this._selectedRegion) return html``;
      ...
      return html` <wl-title
          level="4"
          style="font-size: 17px; margin-top: 20px;"
          >Models for ${this._selectedRegion.name}</wl-title
        >
        ${this._loadingSetups || this._loading
          ...
          : Object.keys(this._categorizedMatchingSetups).map(
              (category: string) => html` <wl-expansion
                name="models"
                @mouseleave="${this._clearPreview}"
              >
                <span slot="title">${category} models</span>
                <span slot="description"
                  >${this._categorizedMatchingSetups[category].length} setups
                  found</span
                >
                ${this._categorizedMatchingSetups[category].map(
                  (setup: ModelConfigurationSetup) => html` <a
                    href="${this._getModelURL(setup.id)}"
                    class="no-decorator"
                  ...
                  >
                    <wl-list-item class="active">
                      ...
                      <wl-title level="4" style="margin: 0;"
                        >${setup.label}</wl-title
                      >

                      <div>
                        <b>Regions:</b>
                        ${setup.hasRegion
                          .map((r) => this._regions[r.id].label)
                          .join(", ")}
                      </div>
                    </wl-list-item>
                  </a>`
        ...
        ${this._matchingSetups.length > 0
          ? html` <wl-title
              ...
              ${this._loadingDatasets
                ...
                : this._matchingModelDatasets.map(
                    (ds: Dataset) => html` <wl-list-item
                      class="active"
                      @click="${() => goToPage("datasets/browse/" + ds.id)}"
                    >
