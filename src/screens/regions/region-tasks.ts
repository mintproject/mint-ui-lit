  ...
  protected render() {
    return html`
      ${this._selectedRegion
        ? html`
            <wl-title level="4" style="font-size: 17px;"
              >Tasks for ${this._selectedRegion.name}</wl-title
            >
            ${this._tasks_loading
              ...
              ? "No tasks for this region"
              : Object.keys(this._scenarios).map(
                  (sid: string) => html`
                    <wl-expansion name="scenarios">
                      <span slot="title">${this._scenarios[sid]}</span>
                      ${this._tasks
                        .filter((el) => el.scenarioid === sid)
                        .map(
                          (el) => html`
                            <wl-list-item
                              class="active"
                              @click="${() => this._goToTask(el)}"
                            >
