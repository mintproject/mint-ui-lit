...
export class DataTransformationList extends connect(store)(PageViewElement) {
            ...
            ${this._dtid
              ...
              : html`
                  <div>
                    ...
                    <p>
                      Select a data transformation from the list to display
                      details.
                    </p>
                  </div>
                `}
  ...
  private _renderDTList() {
    return html`<ul>
      ${Object.values(this._dts).map(
        (dt: DataTransformation) => html` <li>
          <a href="${this._regionid}/datasets/data-transformations/${getId(dt)}"
            >${getLabel(dt)}</a
          >
        </li>`
      )}
  ...
  protected firstUpdated() {
    ...
    store
      .dispatch(ModelCatalogApi.myCatalog.dataTransformation.getAll())
      .then((dts) => {
        this._loadingAll = false;
        this._dts = { ...dts };
      });
    store
      .dispatch(ModelCatalogApi.myCatalog.dataTransformationSetup.getAll())
      .then((dtss) => {
        //TODO
      });
  }
    ...
    if (state.ui && state.ui.selected_datatransformationid) {
      ...
      if (this._dtid != pdtid) {
        ...
        this._selectedDT.setResource(
          this._dtid == "" ? null : ({ id: this._dtid } as DataTransformation)
        );
      }
    }
  }
