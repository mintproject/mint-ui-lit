...
import {
  DATASETS_VARIABLES_QUERY,
  DATASETS_GENERAL_QUERY,
  DATASETS_RESOURCE_QUERY,
  DATASETS_REGION_QUERY,
  DATASET_ADD,
} from "./actions";
...
const datasets: Reducer<DatasetsState, RootAction> = (
  state = INITIAL_STATE,
  action
) => {
  switch (action.type) {
    case DATASETS_VARIABLES_QUERY:
      ...
      state.model_datasets[action.modelid] =
        state.model_datasets[action.modelid] || {};
