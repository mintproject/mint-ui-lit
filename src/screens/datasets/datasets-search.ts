  ...
  protected render() {
    return html`
      <div class="explanation">
        <p>
          The MINT dataset browser allows you to learn about the different
          datasets available in MINT. A single dataset can consist of many files
          (each file is called a resource).
        </p>
        <p>
          In the search bar below you can search datasets in two ways, which you
          can choose on the right. One is to search their descriptions using a
          data source name (eg, GLDAS), keyword (eg crops), and regions (e.g.
          Pongo, Ethiopia). Another is to search their variables (e.g.,
          precipitation).
        </p>
        <p>
          You can then view more detailed information about a dataset by
          clicking on its name. You can also download any of the files
          (resources) in the dataset by clicking on the download link.
        </p>
      </div>
      ...
      <div class="searchResults">
        ${this._datasets
          ? this._datasets.loading
            ? html`<wl-progress-spinner class="loading"></wl-progress-spinner>`
            : html`
                <br />
                <wl-title level="3"
                  >Found ${this._datasets.datasets.length} datasets</wl-title
                >
                  ...
                  return html`
                    <wl-card>
                      <div style="display:flex; justify-content:space-between">
                        ...
                        <a href="${this._region.id}/datasets/browse/${ds.id}"
                          >More Details</a
                        >
                      </div>
                      <div style="display:flex; justify-content:space-between">
                        <wl-title level="5" style="color:#aaa"
                          >id:${ds.id}</wl-title
                        >
                        <span>
                          <span
                            style="color: ${ds.is_cached
                              ? "green"
                              : "lightsalmon"}"
                          >
                            ${ds.is_cached
