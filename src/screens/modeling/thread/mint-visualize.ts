...
export class MintVisualize extends connect(store)(MintThreadPage) {
  ...
  protected render() {
    ...
    let vizurls = getVisualizationURLs(
      this.thread,
      this._task,
    ...
    if (
      this._lastLinks.length != vizurls.length ||
    ...
    let latest_viz_event = getLatestEventOfType(
      ["VISUALIZE"],
      this.thread.events
    );
    console.log(vizurls);
    ...
    return html`
      ...
      ${vizurls && vizurls.length > 0
        ? html`
            <h2>
              Visualization ${responseV ? "of indicator " + responseV : ""}
            </h2>
            ${this._bigViz
              ? html` <div style="display: table; height: 30px;">
                    <wl-icon style="line-height: 30px">report</wl-icon>
                    <span style="display: table-cell; vertical-align: middle;"
                      >This visualization can take a while to load.</span
                    >
                  </div>
                  <wl-button
                    @click="${() => {
                      this._bigViz = !this._bigViz;
                    }}"
                    >Load Visualization</wl-button
                  >`
              : vizurls.map(
                  (vizurl) => html`<iframe src="${vizurl}"></iframe>`
                )}

            <fieldset class="notes">
              <legend>Notes</legend>
              <textarea id="notes">
${latest_viz_event?.notes ? latest_viz_event.notes : ""}</textarea
              >
            </fieldset>
            <div class="footer">
              <wl-button
            ...
            <details>
              <summary>
                Summary of models explored to generate visualizations
              </summary>
              ${this._renderSummary()}
            </details>
          `
        : html`
            ...
            <p>Visualization is a complex problem and a work in progress.</p>
            <p>
              For some models, you will see visualizations here. For other
              models, we recommend that you download the model outputs that
  ...
  _saveNotes() {
    ...
    addThreadEvent(
      getCustomEvent("VISUALIZE", notes) as ThreadEvent,
      this.thread
    );
          ...
          <li>
            ...
            <ul>
                ...
                <ul>
                  <li>
                    ...
                    ${this.thread.response_variables
                      .map((v) => this._variableMap[v].name + " (" + v + ")")
                      .join(", ")}
                  </li>
                  <li>
                    Driving:
              ...
              <li>
                ...
                <ul>
                  ${Object.keys(this.thread.models!).map((modelid: string) => {
                    ...
                    return html` <li>${model.name}</li> `;
                  })}
                </ul>
                <i
                  >Notes:
                  ${getLatestEventOfType(["SELECT_MODELS"], this.thread.events)
                    ?.notes}</i
                >
              </li>
              <li>
                ...
                <ul>
                  ${Object.keys(this.thread.model_ensembles!).map((modelid) => {
                    let model_ensemble = this.thread.model_ensembles![modelid]
                      .bindings as ModelIOBindings;
                    let model = this.thread.models![modelid];
                    return html`
                      ...
                      <ul>
                        ${model.input_files
                          ...
                          .map((io) => {
                            ...
                            return html` <li>${io.name} = ${blist}</li> `;
                          })}
                      </ul>
                    `;
                  })}
                ...
                <i
                  >Notes:
                  ${getLatestEventOfType(["SELECT_DATA"], this.thread.events)
                    ?.notes}</i
                >
              </li>
              <li>
              ...
              <li>
                ...
                <ul>
                  ${Object.keys(this.thread.execution_summary).map(
                    (modelid: string) => {
                      let model = this.thread.models[modelid];
                      let summary = this.thread.execution_summary[modelid];
                      return html`
                        <li>
                          The model setup created ${summary.total_runs}
                          configurations. ${summary.submitted_runs} model runs
                          were submitted, out of which
                          ${summary.successful_runs} succeeded, and
                ...
                <i
                  >Notes:
                  ${getLatestEventOfType(["INGEST"], this.thread.events)
                    ?.notes}</i
                >
              </li>
            </ul>
