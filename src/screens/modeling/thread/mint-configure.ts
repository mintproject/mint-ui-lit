  ...
  constructor() {
    ...
    this.addEventListener("thread-configuration-updated", (e: Event) => {
      ...
      if (this.threadModelSelector.getStatus() === "warning") {
        ...
        this.threadModelSelector.getStatus() === "done" &&
        this.threadDatasetSelector.getStatus() === "warning"
      ) {
    ...
    this.addEventListener("thread-models-updated", (e: Event) => {
      console.log(
        "asdas",
        this.threadModelSelector.getStatus(),
        this.threadDatasetSelector.getStatus()
      );
      this.threadDatasetSelector.requestUpdate();
      if (
        this.threadModelSelector.getStatus() === "done" &&
  ...
  protected render(): TemplateResult {
    return html`
      ...
      <p>
        ...
        choose one or more models and datasets to generate a set of executions
        to be run.
        ${this.permission && this.permission.write && !this.editMode
          ? html`<br />Please click on the
              <wl-icon class="actionIcon">edit</wl-icon> icon inside each
              section to make changes.`
          : ""}
      </p>

      ${this.threadConfig} ${this.threadModelSelector}
