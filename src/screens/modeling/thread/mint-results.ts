...
export class MintResults extends connect(store)(MintThreadPage) {
  ...
  protected render() {
    ...
    if (!cando) {
      return html`
        <p>This step is for monitoring model results.</p>
    ...
    Object.keys(this._executions || {}).map((modelid) => {
      ...
      model.output_files.map((outf) => {
        if (
          ...
          matchVariables(this.thread.response_variables, outf.variables, false)
        ) {
    ...
    return html`
      ${this._notification} ${this.renderDownloadToEmailDialog()}
      <p>
        This step is for browsing the results of the models that you ran
        earlier.
      </p>
        ...
        <ul>
          ${Object.keys(this.thread.execution_summary ?? {}).map((modelid) => {
            ...
            return html` <li>
              ...
              ${submitted_runs
                ? html` <p>
                      Below are the results of all the model executions that run
                      successfully and were completed. The results are shown on
                      the left. The file can be downloaded/viewed by clicking on
                      the link. Click on the RELOAD button if you are waiting
                      for more runs to complete
                    </p>
                    <p>
              ...
              ${false && finished && !submitted && this.permission.write
                ...
                ? html`
                    <p>
                      ...
                      ${summary.ingested_runs || 0} out of ${summary.total_runs}
                      model runs
                    </p>
                  `
                : ""}
                        ...
                        >
                          ...
                          <thead>
                            <tr>
                              ...
                              ${grouped_ensemble.outputs.length > 0
                                ? html`<th
                                    colspan="${grouped_ensemble.outputs.length}"
                                    id="out"
                                  >
                                    Outputs
                                  </th>`
                          ...
                          <!-- Body -->
                          <tbody>
                            ${Object.keys(grouped_ensemble.executions).length ==
                            0
                              ? html` <tr>
                                  <td
                                    colspan="${grouped_ensemble.inputs.length +
                                    grouped_ensemble.params.length +
                                    grouped_ensemble.outputs.length +
                                    1}"
                                  >
...
${latest_ingest_event?.notes ? latest_ingest_event.notes : ""}</textarea
        ...
        : html`
            ${latest_ingest_event?.notes
              ? html`
                  <div class="notepage">
                    ${renderLastUpdateText(latest_ingest_event)}
                  </div>
                `
              : html``}
  ...
  private renderDownloadToEmailDialog(): TemplateResult {
    return html` <wl-dialog
      ...
      size="auto"
    >
      <h3 slot="header">Send model results to email</h3>
      <div slot="content">
        <p>
          Please provide your contact information and details for the requested
          download. <br />
          A download link for the requested data will be sent to the email
          adress entered.
        </p>
        <wl-textfield label="email" type="email" id="email_box"></wl-textfield>
        <p><b>Note:</b> This process can take several minutes.</p>
      </div>
      <div slot="footer" style="padding-top:0px;">
        <wl-button
          ...
          @click=${this.onCancelEmailDialogClicked}
          >Cancel</wl-button
        >
        <wl-button class="submit" @click=${this.onSendEmailDialogClicked}
          >Send</wl-button
        >
      </div>
    </wl-dialog>`;
