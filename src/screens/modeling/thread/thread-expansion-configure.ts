...
export class ThreadExpansionConfigure extends ThreadExpansion {
  ...
  protected _description: TemplateResult = html`General framing for this
  sub-task. The constraints set here will filter the models and datasets
  ...
  protected getStatusInfo(): string {
    if (this.open) return "Edit framing options";
        ...
        <tbody>
              ...
              <div class="formRow">
                <div class="input_half">
                  <input
                    id="thread_from"
                    type="date"
                    value="${toDateString(thread.dates.start_date)}"
                    disabled
                  />
                </div>
                to
                <div class="input_half">
                  <input
                    id="thread_to"
                    type="date"
                    value="${toDateString(thread.dates.end_date)}"
                    disabled
                  />
                </div>
              </div>
          ...
          this.thread.response_variables.length > 0
            ? html` <tr>
                ...
                <td>
                  ${this.indicators && Object.keys(this.indicators).length > 0
                    ? this.indicators[this.thread.response_variables[0]].name
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="formRow">
                                <div class="input_half">
                                    <input id="thread_from" type="date" value="${toDateString(
                                      thread.dates.start_date
                                    )}">
                                </div>
                                to
                                <div class="input_half">
                                    <input id="thread_to" type="date" value="${toDateString(
                                      thread.dates.end_date
                                    )}">
                                </div>
                            </div>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <select id="indicator_selector">
                                ...
                                ${Object.values(this.indicators).map(
                                  (i: Variable) => html`
                                    <option
                                      value="${i.id}"
                                      .selected=${this.thread
                                        .response_variables &&
                                      i.id ===
                                        this.thread.response_variables[0]}
                                    >
                                      ${i.name}
                                    </option>
                                  `
  ...
  stateChanged(state: RootState) {
    ...
    if (state.ui && state.regions) {
      this.regionSelector.loadRegionState(
        state.regions,
  ...
  private getThreadFromForm(): ThreadInfo {
    let nameEl: HTMLInputElement = this.shadowRoot.getElementById(
      "thread_name"
    ) as HTMLInputElement;
    let fromEl: HTMLInputElement = this.shadowRoot.getElementById(
      "thread_from"
    ) as HTMLInputElement;
    let toEl: HTMLInputElement = this.shadowRoot.getElementById(
      "thread_to"
    ) as HTMLInputElement;
    let notesEl: HTMLInputElement = this.shadowRoot.getElementById(
      "thread_notes"
    ) as HTMLInputElement;
    let indicatorEl: HTMLInputElement = this.shadowRoot.getElementById(
      "indicator_selector"
    ) as HTMLInputElement;
