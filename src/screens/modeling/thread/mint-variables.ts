...
export class MintVariables extends connect(store)(MintThreadPage) {
  ...
  protected render() {
    ...
    return html`
      <p>
        This step is for selecting indicators and adjustable variables for your
        analysis. An indicator is an index or a variable of interest that
        results from a model. An adjustable variable indicates the kind of
        inputs that you want to use to drive the results.
      </p>
      ${responseids.length > 0 && !this._editMode
        ? // Response variable chosen already
          html`
            <p>
              Please click on the
              <wl-icon class="actionIcon">edit</wl-icon> icon to make changes.
            </p>
            ...
            <div class="clt">
              <wl-title level="3">
                ...
                <wl-icon
                  ...
                  id="editVariablesIcon"
                  class="actionIcon"
                  >edit</wl-icon
                >
              </wl-title>
              <wl-tooltip
                anchor="#editVariablesIcon"
                    ...
                    ${responseids.length > 0
                      ? responseids.map((responseid) => {
                          ...
                          return html`
                            <li>
                              <a
                                id="response_variable_href"
                                href="${BASE_HREF}variables/${responseid}"
                                >${this._variableMap[responseid]?.name ?? ""}</a
                              >
                              <wl-tooltip
                                anchor="#response_variable_href"
                                .anchorOpenEvents="${["mouseover"]}"
                                .anchorCloseEvents="${["mouseout"]}"
                                fixed
                                anchorOriginX="center"
                    ...
                    ${driverids.length > 0
                      ? driverids.map((driverid) => {
                          return html`
                            <li>
                              <a
                                id="driving_variable_href"
                                href="${BASE_HREF}variables/${driverid}"
                                >${this._variableMap[driverid]?.name ?? ""}</a
                              >
                              <wl-tooltip
                                anchor="#driving_variable_href"
                                .anchorOpenEvents="${["mouseover"]}"
                                .anchorCloseEvents="${["mouseout"]}"
                                fixed
                                anchorOriginX="center"
            ...
            <div class="footer">
              <wl-button
                type="button"
                class="submit"
                @click="${() => store.dispatch(selectThreadSection("models"))}"
                >Continue</wl-button
              >
            </div>
          `
            ...
            <div class="clt">
              ...
              <form id="variablesForm">
                      ...
                      <li>
                        <div class="input_half">
                          ${responseids.length == 0
                            ...
                            : responseids.map((responseid) => {
                                return renderResponseVariables(
                                  responseid,
                                  this._variableMap,
                                  false,
                                  () => {}
                ...
                <div class="footer">
                  ${this._editMode
                    ? html`<wl-button
                        @click="${this._onSetVariablesCancel}"
                        flat
                        inverted
                        >CANCEL</wl-button
                      >`
                  ...
                  <wl-button
                    type="button"
                    class="submit"
                    @click="${this._onSetVariablesSubmit}"
                    >Select &amp; Continue</wl-button
                  >
                </div>
              </form>
            </div>
  ...
  _onSetVariablesSubmit() {
    let form: HTMLFormElement =
      this.shadowRoot!.querySelector<HTMLFormElement>("#variablesForm")!;
    if (formElementsComplete(form, ["response_variable"])) {
      let driving_variable = (
        form.elements["driving_variable"] as HTMLSelectElement
      ).value;
      let response_variable = (
        form.elements["response_variable"] as HTMLSelectElement
      ).value;
      let newthread = {
        ...this.thread,
      };
      newthread.driving_variables = driving_variable ? [driving_variable] : [];
      newthread.response_variables = response_variable
        ? [response_variable]
        : [];
