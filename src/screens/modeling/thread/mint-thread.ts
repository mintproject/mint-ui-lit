...
import {
  TASK_NOT_STARTED,
  getThreadRunsStatus,
  getThreadResultsStatus,
  TASK_DONE,
  TASK_PARTLY_DONE,
  getUISelectedTask,
  getThreadParametersStatus,
  getThreadConfigureStatus,
...
export class MintThread extends connect(store)(MintThreadPage) {
  ...
  @property({ type: Boolean }) private _dispatched_execution_summary: boolean =
    false;
  ...
  private _getBreadcrumbClass(section: string, sectionDoneMap: IdMap<string>) {
    ...
    let cls = "";
    if (this._currentMode == section) cls += " active";
  ...
  private _selectMode(mode: string) {
    ...
    if (this._currentMode) {
      let itemold = this.shadowRoot!.getElementById(
        this._currentMode + "_breadcrumb"
      );
  ...
  stateChanged(state: RootState) {
    ...
    if (thread_id && this.user) {
      if (
        ...
        if (state.modeling.thread?.id && state.modeling.execution_summaries) {
          console.log(
            "Unsubscribing to model execution summary for thread " +
              state.modeling.thread.id
          );
          for (let modelid in (state.modeling.execution_summaries ?? {})[
            state.modeling.thread.id
          ] ?? {}) {
            let summary: ExecutionSummary =
              state.modeling.execution_summaries[state.modeling.thread.id][
                modelid
              ];
            summary.unsubscribe();
          }
      ...
      if (
        ...
        if (!this._dispatched_execution_summary) {
          ...
          console.log(
            "Subscribing to model execution summaries for " + thread_id
          );
    ...
    if (state.modeling && state.ui && state.ui.selected_thread_id == null) {
      ...
      if (state.modeling.thread?.id && state.modeling.execution_summaries) {
        console.log(
          "Unsubscribing to model execution summary for " +
            state.modeling.thread.id
        );
        for (let modelid in state.modeling.execution_summaries[
          state.modeling.thread.id
    ...
    if (
      this.thread &&
      state.modeling.execution_summaries &&
      state.modeling.execution_summaries[thread_id]
    ) {
      this.thread.execution_summary =
        state.modeling.execution_summaries[thread_id];
      this._setSectionStatusMap();
      if (!state.ui.selected_thread_section)
        this._selectMode(this._getNextMode());
  ...
  timeChanged(oldsection: any, newsection: any) {
    ...
    if (oldsection && !newsection) return true;
    if (oldsection.time != newsection.time) return true;
    return false;
  }
}
