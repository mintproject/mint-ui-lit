...
import {
  queryDatasetsByVariables,
  loadResourcesForDataset,
...
export class MintDatasets extends connect(store)(MintThreadPage) {
  ...
  private _comparisonFeatures: Array<ComparisonFeature> = [
    {
      ...
      fn: (ds: Dataset) => html`
        <a target="_blank" href="${this._regionid}/datasets/browse/${ds.id}"
          >Dataset Profile</a
        >
      `,
    },
    ...
    },
    {
      name: "Source",
      fn: (ds: Dataset) =>
        html`<a href="${ds.source.url}">${ds.source.name}</a>`,
    },
    {
      name: "Source Type",
  ...
  protected render() {
    ...
    let noModelHasBeenSelected: boolean =
      !this.thread.models || Object.keys(this.thread.models).length == 0;
    ...
    let latest_data_event = getLatestEventOfType(
      ["SELECT_DATA"],
      this.thread.events
    );
    ...
    if (noModelHasBeenSelected) {
      return html` <p>
          This step is for selecting datasets for each of the models that you
          selected earlier.
        </p>
        Please select model(s) first`;
    }
    return html`
      <p>This datasets match your filters: ${this.datasetSelector}</p>
      <p>
        This step is for selecting datasets for each of the models that you
        selected earlier.
      </p>
      ...
      <div class="clt">
        ...
        ${(Object.keys(this.thread.models) || []).map((modelid) => {
          ...
          let ensembles: ModelIOBindings =
            this.thread.model_ensembles![modelid].bindings || {};
                ...
                <ul>
                  <li>
                    ${fixed_inputs.length == 0
                      ? html`No pre-selected datasets were needed for this
                        model.`
                      : html`
                      ...
                      <ul>
                        ${input_files.map((input) => {
                          ...
                          if (
                              ...
                              <li>
                                ...
                                <ul>
                                  ${bindings.map((binding) => {
                                    ...
                                    let num_total_resources =
                                      dataslice.total_resources ?? 0;
                                    return html`
                                      <li>
                                        <a
                                          target="_blank"
                                          href="${this
                                            ._regionid}/datasets/browse/${dataslice
                                            .dataset
                                            .id}/${this.getSubregionId()}"
                                              ...
                                              ${this.permission.write
                                                ? html` -
                                                    <a
                                                      ...
                                                      @click="${() => {
                                                        this._loadDatasliceResources(
                                                          dataslice
                                                        );
                                                        this._selectDataResources(
                                                          dataslice,
                                                          true
                                                        );
                                                      }}"
                            ...
                            let queriedInputDatasetStatuses = (this
                              ._queriedDatasets[modelid] || {})[input.id!];
                            if (queriedInputDatasetStatuses) {
                              let loading = queriedInputDatasetStatuses.loading;
                              let queriedInputDatasets =
                                queriedInputDatasetStatuses.datasets;
                              let inputtype = input.type.replace(/.*#/, "");
                              let dtypeMatchingInputDatasets = (
                                queriedInputDatasets || []
                                  ...
                                  ${loading
                                    ...
                                    : html`
                                        ...
                                        >
                                          ...
                                          <tbody>
                                            ${(queriedInputDatasets || []).map(
                                              (dataset: Dataset) => {
                                                let matched = matchVariables(
                                                ...
                                                if (
                                                  ...
                                                  dtypeMatchingInputDatasets.indexOf(
                                                    dataset
                                                  ) >= 0
                                                ) {
                                                  return html`
                                                    <tr>
                                                        <td><input class="${this._valid(
                                                          modelid
                                                        )}_${this._valid(
                                                    input.id!
                                                  )}_checkbox" 
                                                            type="checkbox" data-datasetid="${
                                                              dataset.id
                                                            }"
                                                            ?checked="${
                                                              (
                                            ...
                                            ${queriedInputDatasets.length -
                                              dtypeMatchingInputDatasets.length >
                                            0
                                              ? html`
                                                  <tr>
                                                    <td
                                                    ...
                                                    >
                                                      ...
                                                      >
                                                        ...
                                                        ${queriedInputDatasets.length -
                                                        dtypeMatchingInputDatasets.length}
                                                        datasets that matched
                                                        the input variables, but
                                                        not input datatype
                                                        (${inputtype}). They
                                                        might need some data
                                                        transformation.
                                                      </a>
                                                    </td>
                                                  </tr>
                                        ...
                                        <div class="footer">
                                          <wl-button
                                            ...
                                            outlined
                                            @click="${() =>
                                              this._compareDatasets(
                                                modelid,
                                                input.id!
                                              )}"
                                            >Compare Selected Data</wl-button
                                          >
                                          <div style="flex-grow: 1">&nbsp;</div>
                                        </div>
                                      `}
                                  ${this._inputDT[input.id] &&
                                  this._inputDT[input.id].length > 0
                                    ? html`
                                        You can also use the following
                                        <b>data transformations</b> to generate
                                        <b>${input.name}</b>:
                                        <p></p>
                                        <table
                                          class="pure-table pure-table-striped"
                                        >
                                          ...
                                          <tbody>
                                            ${this._inputDT[input.id].map(
                                              (dt: DataTransformation) => html`
                                                <tr>
                                                    <td>
                                                        <input class="${this._valid(
                                                          modelid
                                                        )}_${this._valid(
                                                input.id!
                                              )}_dt_checkbox" 
                                                            type="checkbox" data-transformationid="${
                                                              dt.id
                                                            }"
                                                            ?checked="${false}"></input>
                                                    </td>
        ...
        ${this.permission.write && (!done || this._editMode)
          ? html`
              ...
              <fieldset class="notes">
                <legend>Notes</legend>
                <textarea id="notes">
${latest_data_event?.notes ? latest_data_event.notes : ""}</textarea
                >
              </fieldset>
            `
          : html`
              ...
              ${latest_data_event?.notes
                ? html`
                    <div class="notepage">
                      ${renderLastUpdateText(latest_data_event)}
                    </div>
                  `
                : html``}
                    ...
                    <tr>
                        ...
                        ${this._datasetsToCompare.map((model) => {
                          return html`
                            <th
                              .style="width:${100 /
                              this._datasetsToCompare.length}%"
                            >
                              <b>${model.name}</b>
                            </th>
                          `;
                      ...
                      return html`
                        <tr>
                          ...
                          ${this._datasetsToCompare.map((ds) => {
                            return html` <td>${feature.fn(ds)}</td> `;
                          })}
                        </tr>
                      `;
                    })}
        ...
        <wl-dialog class="comparison" fixed backdrop blockscrolling id="resourceSelectionDialog">
            ...
            <div slot="content" style="height:500px;overflow:auto">
                <table class="pure-table pure-table-striped">
                    ...
                    <tbody>
                        ${(
                          (this._selectResourcesData || ({} as Dataset))
                            .resources || []
                        ).map((resource) => {
                          return html`
                                <tr>
                                    <td>
                                        <input class="checkbox" type="checkbox" data-resourceid="${
                                          resource.id
                                        }"
                                            ?checked="${
                                              resource.selected
                ...
                ${
                  !(this._selectResourcesData || {})["resources_loaded"]
                    ? html`
            ...
            <div slot="footer">
                <wl-button ?disabled="${this._waiting}" 
                        @click="${
                          this._closeResourceSelectionDialog
                        }" inverted flat>Close</wl-button>
                <wl-button type="button" class="submit" ?disabled="${
                  this._waiting
                }"
  ...
  _getDatasetSelections(modelid: string, inputid: string) {
    ...
    this.shadowRoot!.querySelectorAll(
      "input." + this._valid(modelid) + "_" + this._valid(inputid) + "_checkbox"
    ).forEach((cbox) => {
      let cboxinput = cbox as HTMLInputElement;
      let datasetid = cboxinput.dataset["datasetid"];
      if (cboxinput.checked) {
        this._queriedDatasets[modelid!][inputid].datasets.map(
          (dataset: Dataset) => {
            if (dataset.id == datasetid) {
              ...
              selected_datasets[sliceid] = {
                ...
                selected_resources: dataset.resources.filter(
                  (res) => res.selected
                ).length,
  ...
  _getDataTransformationSelections(modelid: string, inputid: string) {
    ...
    this.shadowRoot!.querySelectorAll(
      ...
      if (cboxinput.checked) {
        selected_datatransformations[transformationid] = this._removeUndefined(
          this._dataTransformations[transformationid]
        );
  ...
  _loadDatasetResources(dataset: Dataset) {
    let dates =
      this.thread.dates || this.task.dates || this.problem_statement.dates;
    if (!dataset.resources_loaded) {
      let req = loadResourcesForDataset(
        dataset.id,
        dates,
        this._task_region,
        this.prefs.mint
  ...
  _selectDataResources(
    dataslice: Dataslice | Dataset,
  ...
  async _submitDatasetResources() {
    ...
    dialog.querySelectorAll("input.checkbox").forEach((cbox) => {
      ...
      if (resid) resource_selected[resid] = cboxinput.checked;
    });
    ...
    if (this._selectResourcesImmediateUpdate) {
      ...
      await selectThreadDataResources(
        this._selectResourcesData.id,
        resource_selected,
        this.thread.id
      );
      this._waiting = false;
    ...
    if (mainSelection && mainSelection.length > 0) {
      for (let i = 0; i < mainSelection.length; i++) {
        if (
          mainSelection[i].getAttribute("data-datasetid") ===
          this._selectResourcesData.id
        ) {
          console.log("match!");
  ...
  _compareDatasets(modelid: string, inputid: string) {
    ...
    this._datasetsToCompare = Object.values(dataslices).map(
      (slice) => slice.dataset
    );
    ...
    Object.keys(this.thread.models!).map((modelid) => {
      ...
      model.input_files
        ...
        .map((input) => {
          ...
          Object.values(this._getDatasetSelections(modelid, inputid)).forEach(
            (ds) => {
  ...
  async _selectThreadDatasets() {
    ...
    let model_dt_ensembles: ModelEnsembleMap =
    ...
    Object.keys(this.thread.models!).map((modelid) => {
      ...
      model.input_files
        ...
        .map((input) => {
          ...
          // -If so, return
          let current_data_ensemble: string[] = (this.thread.model_ensembles![
            modelid
          ] || {})[inputid];
          if (
            !this._editMode &&
          ...
          let new_datatransformations = this._getDataTransformationSelections(
            modelid,
            inputid
          );
      ...
      Object.keys(this._models || {}).map((modelid) => {
        ...
        model.input_files
          ...
          .map((input) => {
            ...
            if (
              !this.thread.model_ensembles![modelid] ||
              ...
              this.thread.model_ensembles![modelid].bindings[input.id!].map(
                (datasliceid) => {
                  let dataslice = this.thread.data![datasliceid];
                  this._queriedDatasets[modelid][input.id!].datasets.push(
                    dataslice
                  );
                }
              );
  ...
  stateChanged(state: RootState) {
    ...
    if (this.thread && this.thread.models != this._models) {
      ...
      if (this._models) {
          ...
          Object.values(this._models).forEach((m: Model) => {
            (m.input_files || []).forEach((i) => {
              if (!this._loading[i.id] && !this._dsInputs[i.id]) {
                ...
                store
                  .dispatch(
                    ModelCatalogApi.myCatalog.datasetSpecification.get(i.id)
                  )
                  .then((ds: DatasetSpecification) => {
                    this._dsInputs[ds.id] = ds;
                    this._loading[ds.id] = false;
                    (ds.hasDataTransformation || []).forEach((dt) => {
                      if (
                        !this._loading[dt.id] &&
                        !this._dataTransformations[dt.id]
                      ) {
                        this._loading[dt.id] = true;
                        store
                          .dispatch(
                            ModelCatalogApi.myCatalog.dataTransformation.get(
                              dt.id
                            )
                          )
                          .then((DT) => {
        ...
        this.queryDataCatalog();
        if (this.thread.id != thread_id) this._resetEditMode();
      }
    }
