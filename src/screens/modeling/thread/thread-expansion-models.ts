...
export class ThreadExpansionModels extends ThreadExpansion {
    ...
    protected firstUpdated(): void {
        ...
        let req3 = store.dispatch(ModelCatalogApi.myCatalog.softwareVersion.getAll());
        let req4 = store.dispatch(ModelCatalogApi.myCatalog.modelConfiguration.getAll());
    ...
    public getStatus () : StatusType {
        ...
        if (this.thread.models && Object.keys(this.thread.models).length > 0) return "done";
        return "warning";
    }
    ...
    async save () {
        let models = [];
        let selectedModels : ModelConfigurationSetup[] = this.modelSelector.getSelectedModels()
        ...
        await cacheModelsFromCatalog(models, 
            this.allSoftwareImages, this.allConfigs, 
            this.allVersions, this.allModels);
    ...
    protected onThreadChange(thread: Thread): void {
        ...
        if (thread && thread.response_variables && thread.response_variables.length > 0) {
