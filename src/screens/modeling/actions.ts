...
import {
  ...
  datasliceFromGQL,
  threadModelExecutionSummaryFromGQL,
} from "../../util/graphql_adapter";
...
export const PROBLEM_STATEMENTS_LIST_SUBSCRIPTION =
  "PROBLEM_STATEMENTS_LIST_SUBSCRIPTION";
...
export const THREAD_EXECUTION_SUMMARY_SUBSCRIPTION =
...
export interface ProblemStatementsActionListSubscription
  extends Action<"PROBLEM_STATEMENTS_LIST_SUBSCRIPTION"> {
  unsubscribe: Function;
}
export interface ProblemStatementsActionAdd
  extends Action<"PROBLEM_STATEMENTS_ADD"> {
  item: ProblemStatementInfo;
}
export interface ProblemStatementsActionRemove
  extends Action<"PROBLEM_STATEMENTS_REMOVE"> {
  id: string;
}
export interface ProblemStatementsActionUpdate
  extends Action<"PROBLEM_STATEMENTS_UPDATE"> {
  item: ProblemStatementInfo;
}
...
export interface ProblemStatementsActionSubscription
  extends Action<"PROBLEM_STATEMENT_SUBSCRIPTION"> {
  unsubscribe: Function;
}

export type ProblemStatementsAction =
  | ProblemStatementsActionList
  | ProblemStatementsActionAdd
  | ProblemStatementsActionRemove
  | ProblemStatementsActionUpdate
  | ProblemStatementsActionDetails
  | ProblemStatementsActionSubscription
  | ProblemStatementsActionListSubscription;

export interface TasksActionList extends Action<"TASKS_LIST"> {
  list: TaskList;
}
export interface TasksActionListSubscription
  extends Action<"TASKS_LIST_SUBSCRIPTION"> {
  unsubscribe: Function;
}
export interface TasksActionAdd extends Action<"TASKS_ADD"> {
  item: Task;
  goalid: string;
  problem_statement_id: string;
}
export interface TasksActionRemove extends Action<"TASKS_REMOVE"> {
  id: string;
  problem_statement_id: string;
}
export interface TasksActionUpdate extends Action<"TASKS_UPDATE"> {
  item: Task;
  problem_statement_id: string;
}
export interface TasksActionDetails extends Action<"TASK_DETAILS"> {
  details: Task;
}
export interface TasksActionSubscription extends Action<"TASK_SUBSCRIPTION"> {
  unsubscribe: Function;
}
...
export type TasksAction =
  ...
  | TasksActionUpdate
  | TasksActionDetails
  | TasksActionSubscription
  | TasksActionListSubscription;

export interface ThreadsActionList extends Action<"THREADS_LIST"> {
  list: ThreadList;
}
export interface ThreadsActionListSubscription
  extends Action<"THREADS_LIST_SUBSCRIPTION"> {
  unsubscribe: Function;
}
export interface ThreadsActionAdd extends Action<"THREADS_ADD"> {
  item: Thread;
  subgoalid: string;
  problem_statement_id: string;
}
export interface ThreadsActionRemove extends Action<"THREADS_REMOVE"> {
  id: string;
  problem_statement_id: string;
}
export interface ThreadsActionUpdate extends Action<"THREADS_UPDATE"> {
  item: Thread;
  problem_statement_id: string;
}
...
export interface ThreadsActionSubscription
  extends Action<"THREAD_SUBSCRIPTION"> {
  unsubscribe: Function;
}
...
export type ThreadsAction =
  ...
  | ThreadsActionSubscription
  | ThreadsActionListSubscription;

export interface ThreadVariablesActionAdd
  extends Action<"THREAD_VARIABLES_ADD"> {
  item: string;
  thread_id: string;
}
export interface ThreadVariablesActionRemove
  extends Action<"THREAD_VARIABLES_REMOVE"> {
  id: string;
  thread_id: string;
}
...
export interface ThreadModelsActionRemove
  extends Action<"THREAD_MODELS_REMOVE"> {
  id: string;
  thread_id: string;
}
...
export interface ThreadDatasetsActionRemove
  extends Action<"THREAD_DATASETS_REMOVE"> {
  id: string;
  thread_id: string;
}

export interface ThreadExecutionsActionList
  extends Action<"THREAD_EXECUTIONS_LIST"> {
  thread_id: string;
  model_id: string;
  loading: boolean;
...
export interface ThreadExecutionSummaryActionDetails
  extends Action<"THREAD_EXECUTION_SUMMARY"> {
  thread_id: string;
  model_id: string;
...
export interface ThreadExecutionSummaryActionSubscription
  extends Action<"THREAD_EXECUTION_SUMMARY_SUBSCRIPTION"> {
  thread_id: string;
  model_id?: string;
  unsubscribe?: Function;
}

export interface ThreadExecutionsActionAdd
  extends Action<"THREAD_EXECUTIONS_ADD"> {
  item: Execution;
  thread_id: string;
}
export interface ThreadExecutionsActionRemove
  extends Action<"THREAD_EXECUTIONS_REMOVE"> {
  id: string;
  thread_id: string;
}
export interface ThreadExecutionsActionRun
  extends Action<"THREAD_EXECUTIONS_RUN"> {
  id: string;
  thread_id: string;
}

export type ThreadAction =
  | ThreadVariablesActionAdd
  | ThreadVariablesActionRemove
  | ThreadModelsActionAdd
  | ThreadModelsActionRemove
  | ThreadDatasetsActionAdd
  | ThreadDatasetsActionRemove
  | ThreadExecutionsActionAdd
  | ThreadExecutionsActionRemove
  | ThreadExecutionsActionRun
  | ThreadExecutionsActionList
  | ThreadsActionListSubscription
  | ThreadExecutionSummaryActionDetails
  | ThreadExecutionSummaryActionSubscription;

export type ModelingAction =
...
type SubProblemListThunkResult = ThunkAction<
  void,
  RootState,
  undefined,
  ProblemStatementsActionList | ProblemStatementsActionListSubscription
>;
      ...
      problems.forEach((problem: any) => {
        if (problem["tasks"]) {
          ...
          problem["tasks"].forEach((t) =>
            (t["threads"] || []).forEach(
              (th) =>
                th.response_variable &&
                th.response_variable.name &&
...
type SubProblemDetailsThunkResult = ThunkAction<
  void,
  RootState,
  undefined,
  ProblemStatementsActionDetails | ProblemStatementsActionSubscription
>;
...
// Get Thread details
type ThreadSubDetailsThunkResult = ThunkAction<
  void,
  RootState,
  undefined,
  ThreadsActionDetails | ThreadsActionSubscription
>;
...
type SubThreadExecutionSummaryThunkResult = ThunkAction<
  void,
  RootState,
  undefined,
  ThreadExecutionSummaryActionDetails | ThreadExecutionSummaryActionSubscription
>;
export const subscribeThreadExecutionSummary: ActionCreator<
  SubThreadExecutionSummaryThunkResult
> =
...
// List Thread Execution Summaries
type ThreadExecutionSummaryThunkResult = ThunkAction<
  void,
  RootState,
  undefined,
  ThreadExecutionSummaryActionDetails
...
// List Thread Runs
type ListExecutionsThunkResult = ThunkAction<
  void,
  RootState,
  undefined,
  ThreadExecutionsActionList
>;
export const listThreadModelExecutionsAction: ActionCreator<
  ListExecutionsThunkResult
> =
  (
    thread_id: string,
    model_id: string,
  ...
  (dispatch) => {
    ...
    APOLLO_CLIENT.query({
      ...
      if (result.errors && result.errors.length > 0) {
        ...
        let executions = result.data.execution.map((ex: any) =>
          executionFromGQL(ex)
        );
        dispatch({
          type: THREAD_EXECUTIONS_LIST,
...
export const getAllThreadExecutionIds = async (
  thread_id: string,
  modelid: string
): Promise<string[]> => {
  let APOLLO_CLIENT = GraphQL.instance(OAuth2Adapter.getUser());
  return APOLLO_CLIENT.query({
    ...
    if (result.errors && result.errors.length > 0) {
      ...
      if (result.data.thread_by_pk.thread_models.length > 0)
        return result.data.thread_by_pk.thread_models[0].executions.map(
          (ex: any) => ex.execution_id
        );
    }
    return null;
  });
...
export const selectThreadDataResources = (
  sliceid: string,
...
export const getMatchingEnsemble = (
  ensembles: Execution[],
  execution: Execution,
  hashes: string[]
) => {
...
export const sendDataForIngestion = (
  threadid: string,
...
export const threadTotalRunsChanged = (
  ...
  if (oldthread == null && newthread != null) return true;
  if (newthread == null) return false;
