...
export class MintApp extends connect(store)(LitElement) {
  ...
  private _getMenuLinks() {
    return html`
      <a
        href="${this._selectedRegion ? this._selectedRegion.id : ""}/home"
        class=${this._page == "home" ? "active" : ""}
      >
        <div style="vertical-align:middle">
          ▶
          ${this._selectedRegion
            ? this._selectedRegion.name.toUpperCase()
            : "Select Region"}
        </div>
      </a>
      ${!this.user || !this._selectedRegion
        ...
        : html`
            ...
            >
            <a
              href="${this._selectedRegion.id}/models"
              class=${this._page == "models" ? "active" : ""}
              >Prepare Models</a
            >
            <a
  ...
  private _getMenuButtons() {
    return html`
      <wl-button flat inverted @click="${() => goToPage("home")}">
        ${this._selectedRegion ? "Change Region" : "Select Region"}
      </wl-button>
      ${!this.user || !this._selectedRegion
        ? !this.user
          ? html`
              <wl-button flat inverted @click="${this._showLoginWindow}"
                >Log in to see more</wl-button
              >
            `
          : ""
        : html`
  ...
  private _getPageTitle() {
    ...
    let region = this._selectedRegion
      ? this._selectedRegion.name.toUpperCase()
      : "SELECT REGION";
        ...
        <wl-nav>
          <div class="small-screen" slot="left">
            ...
            <wl-popover
              ...
              anchor="#breadcrumbs-menu-button"
              fixed
                  ...
                  >
                    ...
                    >
                      <wl-button
                        flat
                        inverted
                        class="message-button ${this._page == "messages"
                          ? "selected"
                          : ""}"
  ...
  _renderConfigureUserDialog() {
    return html`
      <wl-dialog id="configDialog" fixed backdrop blockscrolling>
        <h3 slot="header">Configure your MINT account</h3>
        <div slot="content">
          <p></p>
          <wl-label for="user-config-region">Default region</wl-label>
          <wl-select
            name="Default Region"
            value="${this._mainRegion}"
            id="user-config-region"
          >
            <option value="">None</option>
            ${this._topRegions
              ? this._topRegions.map(
          ...
          <div style="margin-top: 4px;">
            <wl-radio
              id="gpublic"
              name="graph"
              ?checked=${!this._defGraph}
            ></wl-radio>
            <wl-label for="gpublic" style="padding: 5px;">
              Public graph (mint@isi.edu)
            </wl-label>
          </div>
          <div style="margin-top: 4px;">
            <wl-radio
              id="gpersonal"
              name="graph"
              ?checked=${this.user && this.user.email == this._defGraph}
            ></wl-radio>
            <wl-label for="gpersonal" style="padding: 5px;">
              My graph
            </wl-label>
          </div>
        </div>
        ...
        <div slot="footer">
          <wl-button @click="${this._onConfigCancel}" inverted flat
            >Cancel</wl-button
          >
          <wl-button @click="${this._onConfigSave}" class="submit"
            >Save</wl-button
          >
        </div>
      </wl-dialog>
  ...
  _onConfigSave() {
    let inputRegion: Select = this.shadowRoot.getElementById(
      "user-config-region"
    ) as Select;
    let inputPublicGraph: Select = this.shadowRoot.getElementById(
      "gpublic"
    ) as Select;
    let inputPrivateGraph: Radio = this.shadowRoot.getElementById(
      "gpersonal"
    ) as Radio;
    let notification: CustomNotification =
      this.shadowRoot.querySelector<CustomNotification>(
  ...
  stateChanged(state: RootState) {
    ...
    if (!state.regions || !state.regions.top_region_ids) {
      ...
      if (
        regionid &&
        !this._dispatchedSubRegionsQuery &&
        (!state.regions.sub_region_ids ||
          !state.regions.sub_region_ids[regionid])
      ) {
